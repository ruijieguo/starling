...
void PerceptionStateStore::upsert(const PerceptionStateRow& row) {
    ...
    const std::string theme_n = schema::normalize_theme(row.theme_id);  // grounding symmetry
    const char* sql =
    ...
    bind_sv(h.get(), 3, theme_n);        bind_sv(h.get(), 4, row.state_dim);
...
std::optional<PerceptionStateRow> PerceptionStateStore::last_known(
    ...
    bind_sv(h.get(), 3, theme_n); bind_sv(h.get(), 4, state_dim); bind_sv(h.get(), 5, as_of);
...
std::vector<PerceptionStateRow> PerceptionStateStore::perceived_for_theme(
    ...
    StmtHandle h(raw);
    bind_sv(h.get(), 1, tenant); bind_sv(h.get(), 2, cognizer);
    bind_sv(h.get(), 3, theme_n); bind_sv(h.get(), 4, as_of);
...
std::string PerceptionStateStore::latest_actual(
    ...
    StmtHandle h(raw);
    bind_sv(h.get(), 1, tenant); bind_sv(h.get(), 2, theme_n);
...
std::string PerceptionStateStore::dim_for_theme(
    ...
    StmtHandle h(raw);
    bind_sv(h.get(), 1, tenant); bind_sv(h.get(), 2, theme_n); bind_sv(h.get(), 3, as_of);
    const int rc = sqlite3_step(h.get());
    if (rc == SQLITE_ROW) return col_text(h.get(), 0);
    if (rc == SQLITE_DONE) return "";
