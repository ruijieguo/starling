...
按顺序照做即可——每个代码块从仓库根目录直接复制粘贴运行。**你不需要系统级的 CMake / Ninja**:它们在第 2 步作为 pip 包装进 venv。系统层面只需要一个 C++ 编译器(和 git)。
...
- C++ 核心链接 **SQLite、OpenSSL、libcurl**,通常系统已自带(macOS:随 Xcode Command Line Tools)。Linux 装开发头文件:`sudo apt install libsqlite3-dev libssl-dev libcurl4-openssl-dev`。`nlohmann/json` 在构建时自动拉取。若后续构建报某个头文件缺失,补装对应的 -dev 包即可。
...
`--config-settings=build-dir=build` 保留一个持久的 `build/` 目录,供 `ctest`(以及 C++ 重建循环)使用:
...
```bash
ctest --test-dir build     # C++(GoogleTest),约 500 个
pytest                     # Python,约 540 个
```

到此即可。`from starling import _core`(已绑定的 C++ 核心)与 `starling.Memory` 门面都能用了;`python examples/quickstart.py` 跑一个离线端到端示例。

**改了 C++ 源 / `migrations/` / 绑定之后**,重新编译并刷新 editable 扩展(否则 pytest 可能加载到陈旧的 `_core.so`):
...
**原理:** [scikit-build-core](https://scikit-build-core.readthedocs.io/)(`pyproject.toml` 里声明的构建后端)驱动 CMake + Ninja 编译 C++ 核心与 `pybind11` 模块 `starling._core`。`migrations/*.sql` 在编译期内嵌进二进制,故 schema 随核心走——运行时没有单独的 migration 步骤。第 3 步同时产出持久的 `build/`(给 `ctest` 与刷新循环)和 editable 安装(给 `import starling`)。
