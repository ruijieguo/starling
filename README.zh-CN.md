...
**前置:** C++20 编译器、CMake ≥3.27、Ninja、SQLite ≥3.46、OpenSSL、libcurl、Python ≥3.11。(`nlohmann/json` 若未找到会自动拉取。)macOS/Homebrew:`brew install cmake ninja sqlite openssl@3 curl`。

**构建系统如何工作:** [scikit-build-core](https://scikit-build-core.readthedocs.io/) 是 Python 构建后端(在 `pyproject.toml` 声明)。它驱动 CMake + Ninja 编译 C++ 核心与 `pybind11` 扩展模块 `starling._core`(CMake flag `-DSTARLING_BUILD_PYTHON=ON`)。`migrations/` 下的 SQL 文件在**编译期内嵌**进核心,故 schema 随二进制走——运行时没有单独的 migration 步骤。`pip install -e ".[dev]"` 产出一个 *editable* 安装,其 import 钩子可自动重建扩展;C++ 目标也可直接用 CMake 构建(见下)以跑 `ctest`。
...
pip install -e ".[dev]"          # scikit-build 编译 _core 扩展
pytest tests/python
```

> 注:改动 C++ 源、migration 或绑定后,用 `cmake --build build && cmake --install build --prefix .venv/lib/python<ver>/site-packages` 刷新 editable 扩展,再重跑 pytest——单独 `pip install -e .` 可能因 scikit-build 的 glob 缓存留下陈旧的 `_core.so`。
