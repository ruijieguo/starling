...
任何对话生成、规划、协商均先调用 `perspective_take(other)` 取对方视角再决策。`filter_by_frontier` 依赖 [Cognizer Hub](v24_08_cognizer.md) 维护的 KnowledgeFrontier；`neocortex.query` 依赖 [Neocortex](v24_07_neocortex.md) 的 holder 子图族。
...
**perspective_take**：算子名。输入 `(target, time)`，输出 `Context(visible, target_beliefs, cg)`。依赖 [Cognizer Hub](v24_08_cognizer.md) 的 `KnowledgeFrontier` 执行 `filter_by_frontier`，依赖 [Neocortex](v24_07_neocortex.md) 的 holder 子图族执行 `neocortex.query(holder=target)`。
