...
文档中标为 P2、P3、P3+ 的机制不得被 P1 实现团队提前做成通用框架。P1 只证明核心闭环可跑通。
...
Starling 核心采用现代 C++（C++20 起步）实现，理由：

- **性能**：Bus / Validator / ConflictProbe / Replay Scheduler / Retrieval Planner 全部在热路径上，零开销抽象 + RAII 内存管理给出可预期延迟
- **嵌入式集成**：C++ 库可链入任何宿主进程，避免独立服务进程的部署复杂度
- **与底层存储同栈**：seekdb 是 C++、LadybugDB 是 C++，核心无跨语言数据搬运
- **多语言绑定**：通过 pybind11 / nanobind / NAPI / cffi 暴露给 Python / JavaScript / Rust / Go / Java

构建工具链：CMake + vcpkg / conan，最低支持 GCC 11 / Clang 14 / MSVC 19.30。
...
| JavaScript / TypeScript | NAPI / node-addon-api | `npm install starling-memory` | Web 后端、Node.js agent |
| Rust | cxx bridge | crates.io | 高性能集成 |
| Go | cgo | go module | 容器化 agent |
| Java | JNI（按需提供） | maven artifact | 企业集成 |

C/C++ 是一等公民接口；其他语言绑定从 C++ API 自动生成存根。

#### 三种集成形态

| 形态 | 描述 | 部署 | 适用 |
|---|---|---|---|
| **嵌入式库** | 宿主进程链入 libstarling | 单可执行文件 | CLI 工具、桌面应用、嵌入式设备 |
| **进程内绑定** | Python / JS / Java 应用 import starling | 单进程，多语言 | agent runtime、Jupyter |
| **独立服务** | starling-server 进程 + RPC 客户端 | 容器化部署 | 多 agent 共享 memory、大规模应用 |

三种形态共享同一 C++ 核心 + 同一存储层 profile（local-store / dist-store / cloud-store），切换形态不改 schema。

#### 配置统一 JSON

所有配置文件、结构化输入采用 JSON 格式：

- 部署配置 `~/.starling/starling.json`（P2.h 起统一；0600 权限，含 token 与 key，绝不进 git/库/log）
- Adapter plugin 注册元数据
- Bus event payload schema
- PipelineRun 账本

理由：跨语言（C++ / Python / JS / Rust / Go）一等支持；schema 可演化（JSON Schema 校验）；diff/audit 友好（行级 git diff）；与外部工具链（OpenAPI、JSON-LD、AsyncAPI）无缝。

非选 YAML 理由：YAML 在 C++ 生态支持不一致，缩进语义陷阱多，跨语言序列化不一致；TOML 仅适合扁平配置，嵌套结构表达力弱。
...
```
                     ...
                     └───┬───────────────────────────────────────┬─────┘
                         ...
                         ▼                                       ▼
   ┌──────────────────────────────────────────────────────────────────┐
   │  Starling C++ Core (现代 C++20)                                   │
   │   多语言绑定: Python (pybind11) / JS (NAPI) / Rust (cxx) /        │
   │              Go (cgo) / Java (JNI)                                │
   │   集成形态: 嵌入式库 / 进程内绑定 / 独立服务                       │
   │   配置: JSON (~/.starling/starling.json)                          │
   └──────────────────────────────────────────────────────────────────┘
   ┌───────────────────────────────────────────────────────────────────┐
   │                       Statement Bus                                │
   │  ┌──────────┐  ┌──────────────┐  ┌────────┐  ┌──────────────────┐ │
   ...
   │   - local-store: raw SQLite + BLOB 暴力向量（嵌入式；P1/P2 实际落地）│
   │     · SQLite: 关系 + JSON + outbox 同库同事务                       │
   │     · SqliteBlobVectorIndex: 暴力 cosine（VectorIndex seam 后接）    │
   │     · 目标形态 seekdb（关系+向量+FTS）+ LadybugDB（嵌入式图）        │
   │       顺延 P3.b（与 §16.5 口径一致）                                 │
   │   - dist-store: Postgres 18 + pgvector + Apache AGE（顺延 P3.b）    │
   │   - cloud-store: 单云原生 / 混合托管 / 跨云 SaaS 三形态（P3.b）       │
...
- **Substrate Adapter 在最底**。所有持久化经它路由到 local-store / dist-store / cloud-store。
...
- **decay 以 CAS 守护**（实现以等价机制达成，2026-06-10 审计改写）。原设计为 decay emit `statement.decay_candidate` 经 outbox 串行；实现为 `op_decay` 直接 UPDATE，以 `consolidation_state` 条件 CAS（`WHERE consolidation_state='consolidated'`）守护并发（`consolidation_ops.cpp`），后到方读到 state 已变即幂等跳过，无 `decay_candidate` 事件，T5/T8 race 同样消除。
- **Commitment 反向保护**（实现以等价机制达成，2026-06-10 审计改写）。原设计为 decay scheduler in-memory set 消费 `commitment.active_holding` 事件；实现为 `commitment_protection` 表 + decay 候选选取时 SQL EXISTS 反查，`commitment.active_holding` / `commitment.released` 事件仍照常 emit 供审计。
...
2. subsystems_design/04_substrate.md（preflight 与索引）
3. subsystems_design/05_bus.md（写入路径与事件表）
4. subsystems_design/06_engramstore.md（证据底座）
5. subsystems_design/06_hippocampus.md（VOLATILE 入口）
6. subsystems_design/13_retrieval.md（basic_retrieve P1 闭环）
7. 主文档 §14 端到端场景流程
8. 主文档 §15 路线图（M0.0 到 M0.7）

P2+ 才需要读：subsystems_design/05_governance.md、subsystems_design/07_neocortex.md（CONSOLIDATED 落地）、subsystems_design/08_cognizer.md、subsystems_design/09_tom.md、subsystems_design/10_replay.md、subsystems_design/11_reconsolidation.md、subsystems_design/12_prospective.md。
...
- `cognizers 1 : 1 knowledge_frontier Container`（实现以等价机制达成，2026-06-10 审计改写：KnowledgeFrontier 由 `cognizer_presence_log` + `cognizer_frontier_facts` 两表作计算视图实现，`containers.kind` 的 `knowledge_frontier` 允许值保留但未物化为 Container 行）
...
- **P2 Projection Index**：7 类专用投影（idx_holder_state_time / idx_holder_subgraph / idx_entity_statement / idx_salience_hot / idx_commitment_due / idx_common_ground / idx_vector_payload），由 outbox subscriber 异步物化，详见 subsystems_design/04_substrate.md。
- **实现补记（2026-06-10 审计回写）**：上述 7 类投影在实现中落为 7 张 `proj_*` 读模型表（proj_holder_state_time / proj_holder_subgraph / proj_entity_statement / proj_salience_hot / proj_commitment_due / proj_common_ground / proj_vector_payload，migration 0015/0017）；向量另有 `statement_vectors` 表承载 raw embedding 与索引向量（0016），0023 起 statement 域次级表主键统一为 `(tenant_id, stmt_id)`。
...
- `predicate` 受控分级落地（2026-06-10 审计改写）：P2 实现为轻量受控注册表——核心谓词预注册，未注册谓词不拒绝、降级 `review_status=REVIEW_REQUESTED` 入库（见 `include/starling/extractor/predicate_registry.hpp`）；完整受控 URI 命名空间 + Cognizer Hub 治理（注册审批、不允许 LLM 自造谓词的硬拒绝）为 P3。
...
- **未结清 Commitment 保护**：实现为 `commitment_protection` 表，decay 候选选取时 SQL EXISTS 排除（实现以等价机制达成，2026-06-10 审计改写；原设计为 in-memory set 消费事件）。`commitment.active_holding` / terminal 态 `commitment.released` 事件仍 emit 供审计。
- **T5/T8 并发互斥**：实现为 `op_decay` 直接 UPDATE + `consolidation_state` 条件 CAS 守护（`WHERE consolidation_state='consolidated'`），后到方读到 state 已变即幂等跳过（实现以等价机制达成，2026-06-10 审计改写；原设计为 emit `statement.decay_candidate` 经 outbox 对同 stmt_id 串行，该事件未实现）。
...
class Commitment(Statement):
    ...
    trigger: Trigger                       # 触发规约，见 subsystems_design/12_prospective.md
...
子类只是 Statement 的特化，不脱离基类字段。实现现状（实现以等价机制达成，2026-06-10 审计改写）：statements 主表**无 type 列**，分类由 `modality + predicate` 承担；Commitment 经 `commitments` / `commitment_triggers` / `commitment_protection` 三张扩展表落地（migration 0018-0020）；EpisodicEvent / Norm / Skill 的特有字段（participants / deontic_strength / procedure 等）未落库，排 P3。
...
> 实现现状（实现以等价机制达成，2026-06-10 审计改写）：Persona 落地为 `self_model_anchor` 与 `profile_anchor` 两类锚的仲裁（自陈优先）+ 自由 predicate 维度聚合，不是 traits/preferences/competencies/values 固定 key 表；`relationship_styles` 落在 `cognizer_relations.fiske_weights_json`。七维固定 dimension 表为 P3 完整化方向。
...
> BusEvent payload 序列化采用 JSON，与全系统配置/结构化输入格式统一。binary payload（如 Engram content_ciphertext）走 Base64 编码或独立 BlobRef 字段，不入 JSON 主体。
...
| `evidence.redacted` | Compliance Engine（生产路径未实现，见下注） | Retrieval index / Audit |
| `evidence.erased` | Compliance Engine / KMS（生产路径未实现，见下注） | Retrieval index / Audit |
...
> `evidence.redacted` / `evidence.erased` producer 现状（2026-06-10 审计回写）：Compliance Engine / KMS 生产触发路径未实现，当前仅测试 helper `starling.testing.mark_evidence_erased` 产出（NullKms 占位）；真 KMS + crypto_erasure 生产路径排 P3.b（与 dist-store 一起）。
...
| `statement.decay_candidate` | Replay decay（未实现；decay 以直接 UPDATE + CAS 等价达成，见 §3.5，2026-06-10 审计注） | Bus dispatcher（per-stmt 串行） |
...
P2 事件（`commitment.*` 族；原标 P3，P2.c 已实现并 emit，2026-06-10 审计改写）：
...
| `norm.violated` | NormChecker（仍未实现，NormChecker 排 P3） | Affect Buffer（高 salience） / Audit |
...
实现新增事件（设计未列、实现已 emit，2026-06-10 审计回写）：

| 事件 | Producer | 用途 |
|---|---|---|
| `evidence.idempotent_hit` | Bus.append_evidence | 同 content_hash 重复 append 命中幂等 |
| `evidence.no_store_audit` | Bus.append_evidence | NO_STORE 策略下只记审计计数不落 Engram |
| `vector.embedded` | EmbeddingWorker | 异步嵌入完成（M0.9 向量层） |
| `commitment.auto_withdrawn` | PolicyEngine | broken_count 累计达阈自动 WITHDRAWN |
| `commitment.renegotiation_blocked` | PolicyEngine / Validator | RENEGOTIATED 链长超限拒绝 |
| `persona.suspected_diverge` | Container Builder（Persona 仲裁） | self_model_anchor 与 profile_anchor 分歧 |
| `testing.*`（mark_consolidated / mark_evidence_erased） | starling.testing helper | 测试态翻转审计，生产 profile 禁用 |
...
2. Extractor（LLM call，JSON 解析；P2.i 起）
3. Bus.write × 4
...
| **P2** | 所有功能基本完备，局部待优化，支持小规模应用 | 14 周 | 已全部收官（P2.a–P2.m，含收尾 P2.d–P2.f 与 dashboard 三期 P2.k/l/m，详见 roadmap 索引） |
...
**P1（6 周 baseline，单/双人熟练 C++20 + Python 绑定）**

定位：最小系统，主要功能可测试。Schema + 写入闭环 + LLM 抽取 + 冲突探针 + basic_retrieve。

| 内容 | 验收方式 |
|---|---|
| Statement + Cognizer + 六态 ConsolidationState + StatementProvenance + ReviewStatus + EngramRetentionMode 核心 schema；C++20 核心对象 + Python（pybind11）绑定层（TypeScript 绑定经 2026-06-10 审计降级 P3.b/按需；dashboard 是 HTTP 前端非绑定层）；local-store CRUD；outbox 表；ProfileCapability preflight（含 cross_partition_transaction）；最小 PipelineRun 账本 + ExtractionAttempt item 明细；SourceKind/IngestPolicy 与 source adapter metadata；chunk 级 SourceSpanRef（engram_ref/chunk_index/observed_at/source_hash）与序列化推导的 TemporalAnchor；一阶 Statement 抽取 prompt（JSON 解析 + holder_perspective 规则；原 XML strict，P2.i 切 JSON 解析器、xml_parser 已删）；basic_retrieve(holder=self, intent=FACT_LOOKUP) 主表直查 + 最小 RetrievalReceipt | 单元测试 + 50 条手工标注对话样本回归 + outbox 投递/幂等测试 + §14.1 最小变体 smoke test（Alice 宣布 Bob 不再负责 auth，testing helper 将 Statement 标为 CONSOLIDATED 后检索返回 Carol 接手，能查看 extraction attempt、pipeline run 与 retrieval receipt）；13 P1 CRITICAL 用例（详见 §15.3.1） |
...
| 子里程碑 | 内容 | 依赖 |
|---|---|---|
| M0.1 Schema | C++ schema 定义 + JSON-Schema 导出，含 tenant_id、derived_depth、canonical_object_hash_version、chunk 级 SourceSpanRef 与 P1 推导式 TemporalAnchor 序列化 | 无 |
| M0.2 持久化 | local-store CRUD + outbox 表 + consumer_checkpoint + idx_statement_id_tenant + 最小 PipelineRun 表；ExtractionAttempt 作为 extraction run item 表；不实现 step graph | M0.1 |
...
| Prospective Loop | **P2.c 已交付**（原标 P3，2026-06-10 审计改写） | CommitmentEngine 五态机 + PolicyEngine 4 类 Trigger 已落地；ActionPolicyGraph 8 规则、external tool 执行仍 P3 |
| CommonGround pool | P2 已交付基础 + grounding acts writer（acknowledge/repair/withdraw/supersede_ground 含 actor，P2.b/P2.j）；P3 余项 = ExpireGround/Unground + 多方（>2）grounding 细则 | 分阶段（2026-06-10 审计更新） |
...
| ToMDepthEstimator（A-ToM） | **v1 已交付 P2.a**（heuristic 版，2026-06-10 审计改写）；完整 A-ToM 仍 P3 | 完整版需 partner 历史数据 |
| 完整 ScopedWorkGate / RestartGuard / stage_timing | P2+ | P1 单线程不需要 |
| RuntimeHealth 完整仪表盘与自动背压调度 | P3.c（原标 P2，与 roadmap 对齐，2026-06-10 审计改写） | 当前只实现 capability preflight + run ledger；DEGRADED/DRAINING 状态置入与自动背压未接线，归 P3.c |
...
- Projection Index：P2 物化视图索引。P1 走主 Statement 表直查。
...
同步更新主文档、Statement Bus、Neocortex、Replay Scheduler 与 Reconsolidation Engine 的事件表和流程描述。v23 及其子系统文档保持不变，v24 是本轮审阅后的编码前候选版本。
...
子系统拆出：Substrate / Bus / Governance / EngramStore / Hippocampus / Neocortex / Cognizer / ToM / Replay / Reconsolidation / Prospective / Retrieval 共 12 个。
