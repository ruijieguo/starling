...
> **本版**为 P0 编码起点。文档由主文档（本文件，§0 摘要、§1 公理、§2 总览、§3 数据本体、§14 端到端场景、§15 路线图、§16 取舍与风险）+ 11 个子系统文档（位于 `v18_subsystems/` 目录）组成。历史变更记录见 [附录 H](#附录-h)。
...
```
   ...
   └────────────────────────────┬─────────────────────────────────────┘
                        ...
                        Application Layer
   ┌──────────────────────────────────────────────────────────────────┐
   │  Substrate Adapter（5 profile：local-lite / local-graph /         │
   │   cloud-graphiti / letta-bridge / cognee-bridge）                 │
...
- **severe contradict 必产新版**。新版 + supersedes 边 + 旧版 ARCHIVED + outbox 事件四项同事务（local-lite 走原子事务，cloud-graphiti 走 saga 补偿）。
...
| # | 子系统 | 文档 | 职责 |
|---|---|---|---|
| 1 | Substrate Adapter | [v18_04_substrate.md](v18_subsystems/v18_04_substrate.md) | 5 profile 的物理底座抽象，三层隔离 |
| 2 | EngramStore | [v18_06_engramstore.md](v18_subsystems/v18_06_engramstore.md) | verbatim 原档，retention_mode 生命周期 |
| 3 | Hippocampus | [v18_06_hippocampus.md](v18_subsystems/v18_06_hippocampus.md) | 快记忆（VOLATILE）、事件切分、Working Set、Affect Buffer |
| 4 | Neocortex | [v18_07_neocortex.md](v18_subsystems/v18_07_neocortex.md) | 慢记忆（CONSOLIDATED）、holder 子图族、五子区 |

**核心运行时子系统**（7 个）：

| # | 子系统 | 文档 | 职责 |
|---|---|---|---|
| 5 | Statement Bus | [v18_05_bus.md](v18_subsystems/v18_05_bus.md) | 写入入口、Validator、ConflictProbe、事件分发、幂等 |
| 6 | Cognizer Hub | [v18_08_cognizer.md](v18_subsystems/v18_08_cognizer.md) | 主体注册、KnowledgeFrontier、RelationEdge |
| 7 | ToM Engine | [v18_09_tom.md](v18_subsystems/v18_09_tom.md) | 二阶信念追踪、perspective_take、7 Mentalizing Primitives |
| 8 | Replay Scheduler | [v18_10_replay.md](v18_subsystems/v18_10_replay.md) | Online/Idle/Sleep 巩固，自适应遗忘 |
| 9 | Reconsolidation Engine | [v18_11_reconsolidation.md](v18_subsystems/v18_11_reconsolidation.md) | 被回忆即可塑，supersedes 链 |
| 10 | Prospective Loop | [v18_12_prospective.md](v18_subsystems/v18_12_prospective.md) | Trigger + Commitment 5 态 + ActionGuard |
| 11 | Retrieval Planner | [v18_13_retrieval.md](v18_subsystems/v18_13_retrieval.md) | 9 Intent + 7 步规划 + Context Pack 8 标签 |

**横切子系统**（1 个）：

| # | 子系统 | 文档 | 职责 |
|---|---|---|---|
| 12 | Runtime Governance | [v18_05_governance.md](v18_subsystems/v18_05_governance.md) | RuntimeHealth 4 态、PipelineRun 账本、ScopedWorkGate 限流 |

**P0 编码必读顺序**：

1. 主文档 §3 数据本体（schema 与边）
2. v18_04_substrate.md（preflight 与索引）
3. v18_05_bus.md（写入路径与事件表）
4. v18_06_engramstore.md（证据底座）
5. v18_06_hippocampus.md（VOLATILE 入口）
6. v18_13_retrieval.md（basic_retrieve P0 闭环）
7. 主文档 §14 端到端场景流程
8. 主文档 §15 路线图（M0.0 到 M0.7）

P1+ 才需要读：v18_05_governance.md、v18_07_neocortex.md（CONSOLIDATED 落地）、v18_08_cognizer.md、v18_09_tom.md、v18_10_replay.md、v18_11_reconsolidation.md、v18_12_prospective.md。

---

## 3. 数据本体

本节冻结 P0 编码所需的全部数据结构。Statement 是核心原子，承载「谁、在何时、基于何证据、对谁、以何样态与极性、持有何判断」；Container 是 Persona / CommonGround / KnowledgeFrontier 的物化视图，由 Bus 重建；Engram 是不可静默覆盖的证据底座，被 Statement.evidence 指向；BusEvent 是跨子系统的事件信封，envelope 字段固定。本节是 schema 的权威来源，子系统文档中的字段须与此对齐。
...
```
  ...
  │  temporal_anchor (embedded JSON, P0 可空)                   │
  ...
  │                                                              │
  │  INDEX idx_statement_id_tenant (id, tenant_id)  -- P0 必备 │
  └──────────────────┬──────────────────────────────────────────┘
                     │ N
  ...
  │  version (整体 CAS, P0/P1)                                   │
  │  dimension_versions (JSON, P2+)                             │
  │  dimension_sequences (JSON, P2+)                            │
...
- **P0 主表必备**：`idx_statement_id_tenant(id, tenant_id)`，承接 `derived_from` 点查、SUPERSEDES 旧版定位、evidence erasure 传播；缺该索引则降级为全表扫描，Validator 拒绝启动。
- **P2+ Projection Index**：7 类专用投影（idx_holder_state_time / idx_holder_subgraph / idx_entity_statement / idx_salience_hot / idx_commitment_due / idx_common_ground / idx_vector_payload），由 outbox subscriber 异步物化，详见 v18_04_substrate.md。
...
- P0 单 tenant group 默认 `tenant_id="default"`；若发现跨 tenant 成员，写入必须拒绝或进入 `REVIEW_REQUESTED` 分支，不得静默降级为 default。
- P1+ 启用跨 tenant group 时，group 自身必须声明 `tenant_id="cross_tenant"` 或显式协议 tenant；以该 group 为 holder 的 Statement 默认 `review_status=REVIEW_REQUESTED`，直到策略或人工确认归属。
...
- `source_spans` 指向 Engram 内的片段位置；P0 最小只要 `engram_ref + chunk_index + observed_at + source_hash`。
- `temporal_anchor` P0 可空，序列化时按 `source_spans[0].observed_at` 推导；P2+ 多源时持久化。
...
class SourceSpanRef(BaseModel):
    ...
    chunk_index: int
    span_start: Optional[int] = None         # P2+；P0 可空
...
- P0 最小实现只要求 `engram_ref/chunk_index/observed_at/source_hash`；`span_start/span_end/segment_id/segment_map` 在 P2+ 引入。
...
class Commitment(Statement):
    ...
    trigger: Trigger                       # 触发规约，见 v18_12_prospective.md
...
class Container(BaseEntity):
    ...
    version: int                            # 整体 CAS（P0/P1）
    dimension_versions: dict[str, int] = {} # P2+
    dimension_sequences: dict[str, int] = {} # P2+；每维度已消费的最高 outbox sequence
...
`Bus.rebuild_container(container_id, dimension, expected_version, source_refs)` 的 `dimension` 参数必须属于上表合法 key。未声明 dimension 必须拒绝写入并 emit `container.dimension_invalid`。P0/P1 用整体 rebuild + 单 `version` CAS；P2+ 启用 `dimension_versions` 细粒度 CAS，`expected_version` 表示对应 dimension version。

Container 不是事实源：Retrieval 可读 Container，但若整体 version 或 P2+ 对应 `dimension_sequences[dimension]` 落后于相关 Statement outbox sequence，必须对该 dimension 降级直接查 Statement 主表，并异步触发修复。
...
class Engram:
    ...
    segment_map: list[SourceSegment] = []    # P2+；P0 可空
...
SourceAdapter 若执行分块、转录、PDF 解析、episode 合并或 multimodal segment 提取，必须输出 `segment_map`。P0 `direct_api` adapter 可不输出 `segment_map`，Extractor 以 `(engram_ref, chunk_index, Engram.timestamp/content_hash)` 构造最小 `SourceSpanRef`。引用 `segment_id/span_start/span_end` 的 Statement 须在 `segment_map` 中找到对应片段；不能由 LLM 自造 offset / episode id。
...
| `statement.consolidated` | Replay Scheduler | ToM（更新 Persona） |
...
路线图按最小可验证增量排列。P0 冻结核心契约，P1–P4 按认知能力分层叠加，P5 产品化并打通多底座迁移。各阶段均以可量化验收指标为准入/出口，不以功能清单为判据。

阶段依赖链：P0 → P1 → P2 → P3 → P4 → P5。P1 依赖 P0 的 holder 子图数据；P2 依赖 P1 的 CONFLICTS_WITH 边与 KnowledgeFrontier；P3 依赖 P2 的 Replay + Reconsolidation 写路径稳定；P4 依赖 P3 的 AffectVector salience 字段用于检索排序权重；P5 依赖 P4 的完整 Retrieval Planner 接口作为迁移适配层。

总工期（不含 P6 持续演进）：P0（6 周）+ P1（4 周）+ P2（6 周）+ P3（4 周）+ P4（4 周）+ P5（4 周）= 28 周 ≈ 7 个月。P1/P2 可在 P0 后期（M0.7 测试阶段）启动 Schema 预研，实际压缩至 6 个月内。



完整路线图：P0 / P1 / P2 / P3 / P4 / P5。

P0：6 周 baseline（单/双人熟练 Python）。Schema + 写入闭环 + LLM 抽取 + 冲突探针 + basic_retrieve。

| 阶段 | 周数 | 核心交付 | 验收方式 |
|---|---|---|---|
| P0 Spike | 6 周 | Statement + Cognizer + 六态 ConsolidationState + StatementProvenance + ReviewStatus + EngramRetentionMode Pydantic 实现；SQLite 单底座 CRUD；outbox 表；ProfileCapability preflight（含 cross_partition_transaction）；最小 PipelineRun 账本 + ExtractionAttempt item 明细；SourceKind/IngestPolicy 与 source adapter metadata；P0 chunk 级 SourceSpanRef（engram_ref/chunk_index/observed_at/source_hash）与序列化推导的 TemporalAnchor；一阶 Statement 抽取 prompt（XML strict + holder_perspective 规则）；basic_retrieve(holder=self, intent=FACT_LOOKUP) 主表直查 + 最小 RetrievalReceipt | 单元测试 + 50 条手工标注对话样本回归 + outbox 投递/幂等测试 + §14.1 最小变体 smoke test（Alice 宣布 Bob 不再负责 auth，testing helper 将 Statement 标为 CONSOLIDATED 后检索返回 Carol 接手，能查看 extraction attempt、pipeline run 与 retrieval receipt）；35 用例（12 CRITICAL） |
| P1 社会心智 Schema | 4 周 | Cognizer Hub（注册 + 别名归一 + 关系类型）；holder 子图族；KnowledgeFrontier 计算；ConflictProbe + CONFLICTS_WITH 边；perspective_take 算子 v1 | ToMBench 一阶 ToM + FANToM 信息不对称 |
| P2 类脑动力学 v1 | 6 周 | Hippocampus/Neocortex 逻辑分区标签 + Projection Index；Replay Scheduler（周期采样 + 聚类 + 三动作：reinforce/abstract/compress）；模式分离/补全（反相似偏移 + PPR 图游走）；Reconsolidation 原子事务（不覆盖） | LongMemEval 时间 + 更新两类得分提升；模式分离混淆率 < 15%；主表查询不承担五维复合索引 |
| P3 前瞻 + 情感 | 4 周 | Commitment 状态机 + 4 类 Trigger（time/event/state/compound）；Prospective Loop 调度器；AffectVector 五维 + salience 公式；优先级重放权重 | 自建承诺履行集 100 条：detection > 80%，timeliness < 3 turns |
| P4 检索规划器 + 二阶 ToM | 4 周 | Retrieval Planner 完整 7 步 + 9 种 QueryIntent；perspective filter（iterative masking）；Abstention 判定；Context Pack Builder 8 标签；二阶 ToM（nesting_depth=2）全链路；ToMDepthEstimator（A-ToM 风格） | ToMBench / FANToM / SoMi-ToM 全量；二阶 ToM 主动提示 precision > 70%；ToMDepth 估计 accuracy > 60% |
| P5 产品化 + 多底座 | 4 周 | Substrate Adapter 5 profiles（local-lite / graph / cloud-graphiti / letta-bridge / cognee-bridge）；mem0 / Letta / cognee / Graphiti / memU 迁移脚本；评测体系全量跑通；API 文档 + 接入指南 | 用户主观评测 A/B 对照；5 种迁移路径集成测试 |

### 15.2 P0 内部里程碑 M0.0 - M0.7
...
12 CRITICAL 用例为 P0 出货门槛，缺一不可。详细列表见 v18_subsystems/07_v16_eng_review_test_plan.md。
...
P0 不交付：Projection Index（P2）；idx_entity_statement 实体投影（P2）；完整 Retrieval Planner（P4）；完整 RetrievalScopePlan runtime（P4）；ToM 推断（P1 一阶 schema，P4 二阶运行时）；Replay Scheduler（P2，P0 只用 testing helper 模拟）；完整 RuntimeHealth 仪表盘（P1/P2）；完整 SourceAdapter 插件系统（P2/P5）；细粒度 segment_map/span_start/span_end（P2）；Reconsolidation Engine（P2）；Prospective Loop（P3）；完整 ActionPolicyGraph（P5 可选）；CommonGround pool（P1 基础，P3 完整）；Affect Buffer（P3，P0 写入 salience 字段但不参与采样）。
...
memU Rust core + blob 层可作为 EngramStore 物理底座候选，但需先补 per-record encryption key、key shredding、shared Engram refcount。memU 要求 Python ≥ 3.13；宿主系统在 3.11/3.12 时应通过服务边界或 Rust FFI 接入，不做 P0/P1 阻塞依赖。WorkflowStep `requires/produces/capabilities` 与 PipelineRevision → Starling 长任务 step 校验 + ProfileCapability preflight + run revision token；Workflow interceptor strict/non-strict + step error hook → failure_policy(critical/non_fatal/skip_downstream)，非致命失败必须记录 warning 且受 downstream requires 约束；pre-retrieval decision / sufficiency check → Retrieval Planner 的 needs_retrieval 与 receipt 中的 plan_steps/degraded_paths；sufficiency check 的 ENOUGH/MORE → RetrievalReceipt.sufficiency_status，其中 NEEDS_RAW 受 EngramStore retention/visibility gate 约束；route_category→sufficiency→recall_items→sufficiency→recall_resources → RetrievalScopePlan 的 progressive widening 与 stop_after_sufficient；non-propagate 开关 → replay_derived 默认不再触发 Replay 的工程依据。
...
设计选择的评判标准：长期可维护性 > 初始实现复杂度；数据结构层的正确性 > Prompt 工程补丁；可审计性 > 运行时性能（P0 阶段）。凡是通过 schema 约束可以解决的问题，不引入运行时模型推理；凡是通过物理隔离可以解决的问题，不依赖业务逻辑过滤。
...
6. **规模化图查询性能**：嵌套 Statement + 关系边 + PPR 游走会形成深图遍历。缓解：预聚合常用子图（per cognizer + per relation type）；PPR 计算缓存；借 cognee 图层分片经验；P5 做性能压测。
...
8. **Outbox 积压**：at-least-once + 幂等是设计选择，outbox dispatcher 停滞会延迟 Replay/ToM/Prospective。缓解：outbox lag 指标、dead-letter queue、subscriber inbox 去重表、每个 aggregate 的 sequence 单调校验。P0 必备。
...
10. **评测单薄**：50 条手工标注对 LLM 抽取统计无功效；FANToM/ToMBench 推到 P1/P4；P0 阶段数据质量信号来源有限，12 CRITICAL 用例是唯一强门槛。

11. **embedding 策略真空**：P0 用主表 LIKE 查询，样本量 50→5000 条即失效；向量检索策略未定，P2 引入 Projection Index 前无法规模化。

12. **hash version 升级期**：maintenance rebuild 未完成前的双查/保守协议复杂度不低；rebuild 期间 ConflictProbe 需两套 hash 共存查询，升级窗口必须有回滚预案。

13. **failure mode 集缺**：LLM 不可用、SQLite WAL 锁、outbox dispatcher hang 等降级路径未在 P0 测试集中列出；P1 需补齐 LLM 降级模式（batch fallback、timeout、partial extraction 三级）。

### 16.3 P2 准入条件

P2 开始前必须满足：

1. P0 全部 12 CRITICAL 用例绿灯，无豁免。
2. P1 ToMBench 一阶 ToM 基准跑通，FANToM 信息不对称对照集准备完毕。
...
5. P1 ConflictProbe 上线后，CONFLICTS_WITH 边写入必须通过 canonical_conflict_key 唯一性校验，防止同一冲突对多次记录。
6. Projection Index 首次 rebuild 前，必须通过 repair guard（SQLite ground truth vs index count）验证，确认 truncation_suspected 告警机制可触发。

### 16.4 P6 持续演进（无硬性交付节点）

P6 研究方向：群聊 SharedGround 维护；Multi-agent 信任传播；PDDL 形式化 belief base（可选高阶）；神经-符号混合实验。P6 不设出货门槛，P5 结束后按研究进度迭代。

依赖：P6 的群聊 SharedGround 需要 P1 CommonGround pool 作为基础；PDDL 形式化 belief base 需要 P4 Context Pack Builder 稳定后才有足够结构化输入；Multi-agent 信任传播需要 P1 KnowledgeFrontier + P4 ToMDepthEstimator 联合驱动。

### 16.5 未交付项（明确延后）

下表汇总所有在 P0–P4 阶段明确不交付的功能项。延后原因以工程必要性和阶段依赖为准，不以"可选"标签代替具体理由。

| 项 | 推迟到 | 原因 |
|---|---|---|
| Retrieval Planner 完整 7 步 + 9 种 Intent | P4 | P0 只交付 basic_retrieve |
| 完整 RetrievalScopePlan runtime | P4 | P0 receipt 只记录固定 statement_main 字段，不构建 plan 对象 |
| ToM 推断运行时 | P1（一阶 schema）/ P4（二阶运行时） | 分阶段引入 |
| Replay Scheduler | P2 | P0 只用 testing helper 模拟巩固完成 |
| Projection Index + idx_entity_statement | P2 | P0 主表直查，不承担五维复合索引 |
| Reconsolidation Engine | P2 | P0 Statement 不因召回进入可塑窗口 |
| Prospective Loop | P3 | P0 不要求 Commitment 触发调度 |
| CommonGround pool | P1（基础）/ P3（完整 grounding acts） | 分阶段 |
| Affect Buffer 参与采样 | P3 | P0 写入 salience 字段但不参与采样 |
| 完整 ActionPolicyGraph 8 规则 | P5 可选 | P3 仅 ActionGuard 最小子集 |
| 模式补全 CA3 风格 | P2 | 计算重 |
| dimension-level Container CAS | P2+ | P0/P1 整体 rebuild 够用 |
| segment_map / span_start / span_end | P2 | 多模态/PDF 分段才需要 |
| ToMDepthEstimator（A-ToM） | P4 | 需 partner 历史数据 |
| 完整 ScopedWorkGate / RestartGuard / stage_timing | P1+ | P0 单线程不需要 |
| RuntimeHealth 完整仪表盘与自动背压调度 | P1/P2 | P0 只实现 capability preflight + 最小 run ledger |
| 完整 SourceAdapter 插件系统 | P2/P5 | P0 只实现内置 direct_api adapter metadata + conformance stub |
| 外部库原生数据结构直连核心表 | 永不交付 | 只能经 Adapter 映射为 Starling 原生原语 |
| Retrieval P4 多源 fan-out latency budget | P4 | 实际负载成点才能估准默认值 |
...
- ActionGuard：前瞻动作护栏，三件套 allowed_actions / requires_approval / idempotency_window。P0/P1 阶段替代完整 ActionPolicyGraph。
- ActionPolicyGraph：8 规则完整动作策略（P5 可选）。含 Init/Child/Parent/Conditional/MaxCount/Terminal/RequiredBeforeExit/RequiresApproval。
...
- chunk_index：SourceSpanRef 字段，Engram 内片段索引。P0 阶段即 normalized_span。
...
- dimension_versions：Container P2+ 字段，dimension-level CAS。P0/P1 用整体 rebuild + 单 version CAS。
...
- Projection Index：P2+ 物化视图索引。P0/P1 走主 Statement 表直查。
...
- TemporalAnchor：时间锚。默认由 SourceSpanRef.observed_at 推导，不得随导入时间漂移。P2+ 才持久化独立字段。
...
策略级：保持六态机不变；P0 时间表调整 2 周→6 周；v17 是 P0 编码起点。
...
证据约束按 provenance 分级；派生链语义单一化（删 derivation_chain）；provenance 写入即冻结；tenant_id 入主体与陈述 schema；P0 TemporalAnchor 简化。

### v14 → v15

简单性优先原则（§0.2）：ActionPolicyGraph / PipelineStepContract / RetrievalScopePlan / Projection Index / dimension-level Container CAS 都有价值，但只有触发条件出现后才升级。P0/P1 以主表、固定路径、单状态机为主。
...
热路径索引策略：阶段边界 + 物化延迟。Projection Index P0 走主表直查，P2 才引入投影；抽取重试以 extraction_span_key 幂等 upsert。
...
VAD 三轴 AffectVector；trust_priors 加入 Cognizer；polarity 与 modality 正交；canonical_name + aliases 主体归一；EngramStore 独立 BlobStore；模式分离 MAY_OVERLAP_WITH 软边；CommonGround N 元扩展。Statement 显式 5 态 ConsolidationState；Retrieval 新增 Abstention Gate；路线图从 4 阶段扩为 6 阶段（P0-P6）。
