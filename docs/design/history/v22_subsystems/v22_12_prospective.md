...
Prospective Loop 实现"前瞻"，在没有用户 query 的情况下主动想起承诺、触发提醒、检测履行或违约。它承担四项职责：（1）Trigger 类型系统；（2）Commitment 五态机；（3）ActionGuard 动作护栏；（4）Norm 监听。所有 Trigger 命中均经 [Statement Bus](v22_05_bus.md) emit `commitment.fire`，PolicyEngine 不绕过 Bus 直接调用消费者。
...
PolicyEngine 是 [Statement Bus](v22_05_bus.md) 的 publisher 与 subscriber：
...
`modality=COMMITS` 的 Statement 经 Validator 通过后，Bus 自动将其状态由 `created` 转为 `ACTIVE`。转 ACTIVE 时 PolicyEngine emit `commitment.active_holding(commitment_id, related_stmt_ids)`，通知 [Replay Scheduler](v22_10_replay.md) decay scheduler 将关联 stmt_id 加入 in-memory 保护集，阻止相关 Statement 被 ARCHIVED。
...
同一 `commitment_id` 累计进入 BROKEN 状态 `>= MAX_BROKEN_COUNT`（默认 3）次后，下一次 deadline 过期不再转 BROKEN，而是自动转 WITHDRAWN，emit `commitment.auto_withdrawn(reason="chronic_failure")`。同步触发 [Cognizer Hub](v22_08_cognizer.md) 中对应 holder 的 `trust_priors` 下调（具体公式见 §8.3）。设计意图：防止单一 commitment_id 长期占用 supersedes 链与 Replay 资源，语义上反映"反复失信后系统不再跟踪"。
...
- 每个外部动作须携带 `business_idempotency_key`，窗口规则沿用 [Statement Bus](v22_05_bus.md) §5.4。
...
`max_count` 与 [Statement Bus](v22_05_bus.md) §5.4 `business_idempotency_window` 互补：前者限制单次 action graph 内执行次数，后者限制 at-least-once 事件重复。`terminal` 动作结束该 Prospective run，后续动作须新建 `causation_chain`。
...
PolicyEngine 是 [Statement Bus](v22_05_bus.md) 的 publisher 之一，同时也是 Bus 订阅者（订阅 commitment.fulfilled / broken / renegotiated / withdrawn 用于 Trigger 清理和 trust_priors 调整）。所有外发事件经 `Bus.emit`，所有内入事件来自 Bus，满足 §5"所有读写必经 Bus"硬约束。
...
**引用**
- 事件总线契约：[Statement Bus](v22_05_bus.md)
- trust_priors 下调公式：[Cognizer Hub](v22_08_cognizer.md) §8.3
- decay scheduler 例外条款：[Replay Scheduler](v22_10_replay.md) §3.4
- ToM 推断 supersedes 链展开：[ToM Engine](v22_09_tom.md) §9.2
