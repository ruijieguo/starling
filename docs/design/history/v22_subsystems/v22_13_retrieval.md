...
Retrieval Planner 是视角感知检索与心智摘要子系统。它按 `(querier, perspective, intent, goal)` 四元组重构 Context Pack，不是工具堆的 fan-out 封装。对外可见效果上它是纯读模块：不直接修改 Statement state，只以 fire-and-forget 方式 emit `statement.recalled` 事件，由 [Reconsolidation Engine](v22_11_reconsolidation.md) 异步消费决定是否开窗。
...
**perspective filter**
硬约束，必须在语义排序之前执行。依赖 [Cognizer Hub](v22_08_cognizer.md) 的 KnowledgeFrontier 实现 EnigmaToM iterative masking，决定哪些证据对当前 perspective 可见。
...
`statement.recalled` 事件由 Bus 防抖合并，[Reconsolidation Engine](v22_11_reconsolidation.md) 异步消费。频繁召回（短窗口内 ≥ K 次）或与近期 `belief.conflict` 同 key 时开窗；否则只升软 activation，不开窗。Retrieval 主流程不等待。

**Mentalizing Primitives**
META\_BELIEF intent 的嵌套深度由 [Mentalizing Primitives](v22_09_tom.md) 的 ToMDepthEstimator 调制，防止无限递归二阶信念推断。
