# Starling Memory v6:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v6,基于 v5 的架构审查结果,不再扩展认知能力面,而是补齐第一轮 schema/运行时实现会卡住的**可执行契约**:状态枚举、Bus 事务 outbox、Drawer 合规删除、派生候选状态、再巩固原子事务、Container 更新边界、Conflict/幂等 key、Commitment 终态事件与路线图一致性。
>
...
**Starling v6 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
...
│  │ ─────────     │         │ ──────────────────      │                │
│  │ • Episodes    │ ◄─补全─ │ • Semantic (KG, holder子图族) │          │
│  │ • Working Set │         │ • Procedural (Skills)   │                │
│  │ • Affect Buf  │         │ • Norms (规范库)        │                │
│  └───────────────┘         │ • Personae (物化视图)   │                │
│                            │ • CommonGround (物化视图) │              │
│  └────┬──────────┘         └────────────┬────────────┘                │
│       │                                  │                            │
...
更新规则:
- 只有 `Bus.rebuild_container(container_id, expected_version, source_refs)` 能写 Container。
- 使用 CAS:`expected_version` 不匹配则失败并重新读取来源 StatementRefs。
- `container.rebuilt` 事件只表示物化视图刷新,不表示新增事实。
- Retrieval 可读 Container,但若 `last_rebuilt_at` 落后于相关 Statement outbox sequence,必须降级直接查 Statement 图。
...
| **隐私强制** | FORGOTTEN + redaction/crypto erasure 传播到所有 derived | 用户/法务事件 |
...
| 阶段 | 时间 | 目标 | 核心交付 | 验证方式 |
|---|---|---|---|---|
| **P0 Spike** | 2 周 | 可执行契约冻结 | Statement + Cognizer + 六态 ConsolidationState + StatementProvenance + ReviewStatus + DrawerRetentionMode 的 Pydantic 实现与 JSON-Schema 导出;SQLite 单底座 CRUD;outbox 表;一阶 Statement 抽取 prompt(XML strict) | 单元测试 + 50 条手工标注对话样本回归 + outbox 投递/幂等测试 |
...
| **P2 类脑动力学 v1** | 6 周 | CLS 双系统 + 巩固回路 | Hippocampus/Neocortex 逻辑分区标签 + 可选物理投影/索引;Replay Scheduler(周期采样 + 聚类 + 三动作);模式分离/补全(反相似偏移 + PPR 图游走);Reconsolidation 原子事务(不覆盖) | LongMemEval 时间 + 更新两类显著提升;模式分离混淆率 < 15% |
