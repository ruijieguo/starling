...
Substrate Adapter 是 Starling 的存储隔离层，把上层逻辑层（[Statement Bus](v20_05_bus.md)、[Hippocampus](v20_06_hippocampus.md)、[Neocortex](v20_07_neocortex.md) 等）与底层物理存储完全解耦。最小定义：将标准化接口调用路由到当前 profile 对应的物理底座，并在启动阶段通过 preflight 校验确认底座具备所需能力。

它不是 ORM，不做 SQL 方言转译，不是查询计划器，不持有业务缓存，不参与跨分区事务协调（事务补偿由 [Reconsolidation Engine](v20_11_reconsolidation.md) 负责）。

## 输入

- 上层 Bus.write/append_evidence/rebuild_container 调用（含 Statement 主体、Engram payload、Container dimension）
- ProfileCapability 声明文件（来自部署配置）
- 索引重建请求（Projection Index 物化触发）

## 输出

- 持久化结果（行 ID、commit timestamp、affected_rows）
- preflight 校验结果（READY / UNREADY + 缺失能力列表）
- Projection Index 物化视图（按 holder/state/time 等维度分组的查询结果）
...
规则 R3（crypto_erasure 依赖）：
  IF retention_mode=crypto_erasure:
    REQUIRE engram_per_record_key=true
    REQUIRE crypto_erasure=true
    # 不得自动降级为逻辑隐藏

规则 R4（共享 Engram）：
  IF shared_engram_enabled:
    REQUIRE engram_refcount=true OR refcount_split_strategy_provided
...
规则 R8（tenant_id 落库）：
  ...
  # P1 可用 testing adapter 的 final query assertion 校验占位符存在
```
...
`false` 时，[Reconsolidation Engine](v20_11_reconsolidation.md) 的 severe path 必须走 saga 补偿路径：先写新版 Statement 与 supersedes 边，后 emit 事件，最后将旧版标为 ARCHIVED；任一步失败则 emit `reconsolidation.compensated`，旧版保持 CONSOLIDATED。
...
class ProfileCapability(BaseModel):
    ...
    # Engram / 加密擦除
    engram_per_record_key: bool        # per-record 加密 key，crypto_erasure 前置条件
...
Projection Index 由 outbox subscriber 异步物化，不阻塞 `Bus.write` 主事务。Freshness SLA：Online 模式 < 2s，Idle/Sleep 批处理 < 30s；超过 SLA 时 [Retrieval Planner](v20_13_retrieval.md) 降级主表直查或标记结果 stale。

`idx_holder_state_time` 是唯一允许 P2 提前引入的轻量投影。
...
**Logical layer**：[Statement Bus](v20_05_bus.md)、[Hippocampus](v20_06_hippocampus.md)、[Neocortex](v20_07_neocortex.md) 等上层子系统所在的语义操作层，通过标准接口调用 Substrate Adapter，不感知底层物理存储类型。
...
**Projection Index**：从 Statement 主表派生的物化投影索引，P2 起引入，由 outbox subscriber 异步维护，可由主表 + outbox sequence 完全重建，不作为权威事实源。与主表索引（如 `idx_statement_id_tenant`）的区别：主表索引服务于点查和一致性校验，Projection Index 服务于高频 Retrieval 的多维过滤。详见 [Retrieval Planner](v20_13_retrieval.md)。
