# Starling Memory v13:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v13,以 v12 为基线,再次深读 9 个开源代码库(mem0 / Letta / cognee / Graphiti / MemOS / EverOS / memU / mempalace / claude-mem)后补齐仍缺的实施契约:源片段时间锚、归因不可变、实体桥投影、渐进式检索 ScopePlan、Pipeline step 失败策略与 Prospective action policy。v13 不改变 Statement 中心化、holder 子图族、六态生命周期、Replay/Reconsolidation 核心模型,只把源码中可验证的运行时护栏压实到 schema、bus、retrieval 和 roadmap。
>
...
**Starling v13 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
...
class Statement(BaseEntity):
    ...
    source_spans: list[SourceSpanRef]        # v13:Drawer 内片段/episode/segment 级证据锚
    temporal_anchor: TemporalAnchor          # v13:相对时间解析锚,默认来自 source_span
...
class TemporalAnchor(BaseModel):
    anchor_kind: Literal["source_span","episode","drawer_record","message","document","system_now"]
...
| source span 绑定 | 每条 Statement 必须绑定 ≥1 个 `SourceSpanRef`;Extractor 只能引用 DrawerRecord.segment_map 中存在的 segment/offset。Graphiti 式多 episode 事实需记录所有 episode span,并以第一归因 episode 的 observed_at 作为默认 temporal_anchor |
...
class RetrievalScopeStep(BaseModel):
    scope: Literal["working_set","session","trace","main_table","projection",
                   "vector","graph","container","drawer","tom"]
...
- `basic_retrieve` 的 plan 固定为 `mode=basic, steps=[main_table], stop_policy=merge_all`。
...
- Progressive plan 先查 Working Set/session/main_table 等低成本 scope;若 `sufficiency_status=SUFFICIENT` 且 stop_policy 允许,跳过 graph/vector/drawer raw。
...
- `scope_plan` 必须能解释每个 scope 的 filter 来源、holder/group、fallback 与 stop policy;若 plan 因 session/main_table 已足够而短路,`skipped_scopes` 必须列出未查的 graph/vector/drawer。
...
- 完整 RetrievalScopePlan runtime(P4;P0 receipt 可记录固定 main_table plan)
