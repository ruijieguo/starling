# Starling Memory v11:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v11,基于 v10 与 9 个开源代码库的源码级复审(mem0 / Letta / cognee / Graphiti / MemOS / EverOS / memU / mempalace / claude-mem),补齐运行时健康、背压、长任务账本、检索可观测性、抽取尝试账本与底座能力预检。v11 不改变核心认知本体,只把源码中反复验证有效的工程闭环上升为 Starling 的可执行契约。
>
...
**Starling v11 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
...
| evidence 绑定 | 每条 Statement 必须绑定至少 1 个 DrawerRef;多句合并 Statement 必须记录 `span_offsets` 或等价 chunk 指针 |
...
| `pipeline.run_started/completed/failed/cancelled` (v11 新增) | PipelineRun ledger | Ops / Replay / Projection / Audit | 长任务生命周期 |
...
class PipelineRun(BaseEntity):
    ...
    input_hash: str
    idempotency_key: str
    status: Literal["QUEUED","RUNNING","PAUSED","COMPLETED","FAILED","CANCELLED","DEAD_LETTERED"]
...
class RetrievalReceipt(BaseModel):
    ...
    scopes_searched: list[str]              # main_table/vector/graph/container/drawer/tom/working_set
...
| 阶段 | 时间 | 目标 | 核心交付 | 验证方式 |
|---|---|---|---|---|
| **P0 Spike** | 2 周 | 可执行契约冻结 + 最小闭环 | Statement + Cognizer + 六态 ConsolidationState + StatementProvenance + ReviewStatus + DrawerRetentionMode 的 Pydantic 实现与 JSON-Schema 导出;SQLite 单底座 CRUD;outbox 表;`ProfileCapability` preflight;最小 `PipelineRun/ExtractionAttempt` 账本;一阶 Statement 抽取 prompt(XML strict);`basic_retrieve(holder=self, intent=FACT_LOOKUP)` 主表直查 + 最小 `RetrievalReceipt` | 单元测试 + 50 条手工标注对话样本回归 + outbox 投递/幂等测试 + §14.1 最小变体 smoke test: Alice 宣布 Bob 不再负责 auth,检索返回 Carol 接手,并能查看 extraction attempt 与 retrieval receipt |
...
| 子里程碑 | 内容 | 依赖 |
|---|---|---|
| M0.1 Schema | Pydantic 实现 + JSON-Schema 导出 | 无 |
| M0.2 持久化 | SQLite CRUD + outbox 表 + consumer_checkpoint + 最小 `PipelineRun/ExtractionAttempt` 表 | M0.1 |
| M0.3 写入路径 | `Bus.append_evidence` + Drawer + retention_mode 审计 | M0.2 |
| M0.4 抽取 | Extractor + Validator + `extraction_span_key` | M0.3 |
| M0.5 冲突 | ConflictProbe + `canonical_conflict_key` + `SUPERSEDES` 边 | M0.4 |
| M0.6 检索 | `basic_retrieve` + 主表过滤 + evidence 返回 + 最小 `RetrievalReceipt` | M0.5 |
...
| **Letta** | Blocks(工作记忆);Sleeptime 后台任务;Identity ORM(主体注册);**GitEnabledBlockManager**(真 git CLI 版本);Archive(共享原档池);LoadGate / event-loop watchdog | 工作记忆改 perspective 化(holder 标签);sleeptime 升级为 Replay Engine(reconcile/abstract/induce_norm);shared_blocks → CommonGround pool;Identity ORM → Cognizer 迁移起点;LoadGate/watchdog → RuntimeHealth readiness 与前后台 admission gate |
| **cognee** | DataPoint 子类机制;TEMPORAL 时间边(外包 graphiti);**improve() feedback_weight alpha 已实装**(四阶段流水线);forget();run_custom_pipeline + dataset queue slot + access-control capability check | 子类化建立社会心智本体(Statement→Belief/Commitment/Norm...);improve 改为 Reconsolidation 状态机(在线增量);forget 改为降级不删除;custom pipeline → PipelineRun 账本与 profile capability fail-closed |
...
| **memU** | **Rust core(v1.4+)**,Python binding;WorkflowStep requires/produces/capabilities;pre-retrieval decision/sufficiency check;Category summary 增量维护;non-propagate patch;blob 存储层 | compress 动作默认 non-propagate=True;workflow step contract → PipelineRun step validation 与 ProfileCapability;pre-retrieval decision → RetrievalReceipt plan_steps;Rust blob 层作为 Drawer 物理底座候选;Python binding 版本要求需纳入 adapter 成本 |
| **claude-mem** | Hook 生命周期;SQL 主 + 向量副降级;严格 XML 输出;SessionQueueProcessor claim-confirm;HealthMonitor/readiness;RestartGuard;ChromaSync watermark | Hook 接 Starling 写入端;Observation 升级为 Statement;claim-confirm/RestartGuard/watermark → RuntimeHealth、worker lease、Projection rebuild checkpoint;`<private>` tag 边缘剥离 → `visibility / retention_policy` 实现参考 |
| **graphiti / Zep**(v4 新增独立行) | 三时间字段 `valid_at/invalid_at/expired_at`;SagaNode 增量摘要;多 scope search trace(edge/node/episode/community);MinHash/Jaccard 去重;contradiction resolver;DateFilter CNF | 三时间复用为 Starling 四时间制(+ inferred_at);SagaNode → Replay `compress`;多 scope trace → RetrievalReceipt.scopes/candidate_counts/score_breakdown;resolve_edge_contradictions → supersedes 链;加 holder 维度后做子图族 |
