...
|---|---|---|---|---|---|---|---|
| **local-store** | 嵌入式，零服务进程 | SQLite (JSON1+FTS5) | LanceDB | Kuzu | P1 | 单机 | 全部 OSI 许可 |
...
| 图 | Kuzu | 0.10+ | MIT | 文件 | Cypher, PageRank, columnar OLAP |
...
- **Kuzu**：唯一同时满足 (MIT) + (in-process Python) + (Cypher) + (持久化) + (PageRank) 的图引擎；列存图引擎对 N-hop 遍历友好。

### 备选与简化变体

- 量级 ≤ 10⁵ 时可改用 sqlite-vec 把向量塞进 SQLite 同事务（栈从 3 减到 2），向量与 Statement 主表同库同备份。
- Kuzu 上游 2025-10 被归档，源码 MIT 永久可用，社区 fork 包括 RyuGraph、bighorn、LadybugDB；P1 主线先用 Kuzu，上线前可切 RyuGraph（API 兼容）跟随安全补丁。
...
```
P1 → P2:
  [1] P1 端 export_dump（SQLite/Lance/Kuzu）
...
- 引用 [Statement Bus](v21_05_bus.md), [Runtime Governance](v21_05_governance.md), [EngramStore](v21_06_engramstore.md), [Reconsolidation Engine](v21_11_reconsolidation.md)
