# Starling Memory v15.1:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v15.1,以 v15 为基线做 P0 编码前的契约收口。v15.1 不新增认知能力,只修正 schema / Validator / Bus / ConflictProbe 会现场卡住的矛盾,并继续遵守 v15 的简单性原则:能用主表、固定路径、单一前驱字段和少量索引解决的问题,不引入第二套运行时框架。
>
> **v15.1 相对 v15 的修复**(详见 §17.15 v15 → v15.1 修复记录):
>
> *Blocker 级*:
> 1. **证据约束按 provenance 分级** — `evidence ≥1` 不再作为所有 Statement 的硬规则;直接抽取必须有 Drawer evidence/source span,派生 Statement 必须有 `derived_from` 前驱,二者至少一条链路可追溯(§3.2/§5.1)
...
>
> **v15 相对 v14 的修复**(详见 §17.14 v14 → v15 修复记录):
>
> *Major 级*:
> 1. **简单性优先原则** — 新增 §0.2,定义"先固定路径,后泛化;先审计事件,后框架;先主表,后投影"的升级触发条件(§0.2/§16.1)
> 2. **ActionPolicyGraph 降级** — P3 只保留 `ActionGuard` 三规则(allowlist / requires_approval / idempotency_key);完整图式动作编排降为 P5 可选,避免把 Prospective Loop 做成工具工作流系统(§12.5/§16.1)
> 3. **PipelineStepContract 降级** — P0/P1 的 `PipelineRun` 只记录 `pipeline_name/version/status/counters/watermark`;`step_contracts/requires/produces/failure_policy` 移到 P2+ 复杂 pipeline 才启用(§5.6/§16.1)
> 4. **RetrievalScopePlan 降级** — P0/P1 只允许固定 `basic_retrieve` 路径;完整 progressive/parallel/exhaustive scope plan 延后到 P4,避免检索规划器过早泛化(§13.1/§13.4/§16.1)
>
> *Minor 级*:
> 5. **SourceSpan 最小化** — P0 仅要求 `drawer_ref/chunk_index/observed_at/source_hash`;offset/segment_map 多段细粒度锚定移到 Adapter conformance 后启用(§3.2/§3.8/§5.1)
> 6. **路线图去重** — P0 非交付项增加完整 ActionPolicyGraph、PipelineStepContract、dimension-level Container CAS 与高级 SourceSegment,明确简单实现边界(§16.1)
>
...
> 7. **ToM 派生链限流** — `tom_inferred` Replay 降权公式化;派生链超阈值时暂停 ToM 增量推断,防止 ToM→Replay→Persona→ToM 多层环路(§3.10/§5.4/§9.2/§10.2)
...
| 证据锚 | `drawer_ref + chunk_index + observed_at + source_hash` | 多 episode、PDF 分段、转录、多模态或 offset 级擦除需求出现后启用 `segment_map/span_start/span_end` |
| 检索 | 固定 `basic_retrieve(statement_main)` | 多 scope 成本差异、sufficiency 短路或 drawer raw gate 需求出现后启用完整 RetrievalScopePlan |
...
**证据锚适用范围(v14 收敛)**:
- `provenance=user_input` 且直接由 DrawerRecord 抽取的 Statement,必须有 `evidence ≥1`、`source_spans ≥1`;P0 可由 `source_spans[0].observed_at` 自动生成 `temporal_anchor`,P2+ 多源时再持久化独立字段。
...
- P0 最小实现只要求 `drawer_ref/chunk_index/observed_at/source_hash`;`span_start/span_end/segment_id/segment_map` 属于 Adapter conformance 增强,在 P2+ 引入。
- P0 可不暴露独立 `TemporalAnchor` 存储字段,序列化时按 `SourceSpanRef.observed_at` 推导:`anchor_kind="source_span"`、`anchor_time=observed_at`、`resolved_by="metadata"`、`confidence=1.0`、`timezone` 取 DrawerRecord metadata(缺失为 UTC)。P2 启用 segment_map 与多片段聚合后,`TemporalAnchor` 必须独立持久化以承载多源不一致。
...
SourceAdapter 若执行分块、转录、PDF 解析、episode 合并或 multimodal segment 提取,必须输出 `segment_map`。P0 direct_api 可不输出 `segment_map`,Extractor 以 `(drawer_ref, chunk_index, DrawerRecord.timestamp/content_hash)` 构造最小 `SourceSpanRef`。凡引用 `segment_id/span_start/span_end` 的 Statement,必须能在 `segment_map` 中找到对应片段;不能凭 LLM 自造 offset/episode id。若 adapter 无法给出片段级时间,DrawerRecord.timestamp 可作为 fallback,但所有相对时间抽取默认 `review_status=INFERRED_UNREVIEWED`。
...
|---|---|---|---|
| `user_input` | ≥1 条 DrawerRef,必填 | ≥1,必填(P0 chunk 级即可) | 可为空 |
...
直接抽取 Statement 的 `tenant_id` 由 `holder.tenant_id` 派生并写入后不可变;派生 Statement 默认继承直接前驱集合中主前驱的 tenant,跨 tenant 派生默认拒绝。
...
| source span 绑定 | 直接抽取 Statement 必须绑定 ≥1 个 `SourceSpanRef`;P0 可用 `(drawer_ref, chunk_index, observed_at, source_hash)`。若引用 `segment_id/span_start/span_end`,必须能在 DrawerRecord.segment_map 中找到对应片段。Graphiti 式多 episode 事实需记录所有 episode span,并以第一归因 episode的 observed_at 作为默认 temporal_anchor。派生 Statement 不得伪造直接 source span |
...
| 索引 | 字段 | 用途 |
|---|---|---|
| `idx_conflict_lookup` | `(tenant_id, holder, modality, subject, predicate, canonical_object_hash)` | 候选集前置过滤 |
| `idx_temporal_overlap` | `(tenant_id, holder, valid_from, valid_to)` | 时间区间 overlap 检测 |

`canonical_object_hash` 是 Statement 主表派生列,写入时由 `canonicalize_object(stmt.object)` 计算并持久化,禁止每次 ConflictProbe 查询时运行时重算。时间 overlap 使用闭开区间条件:`start <= query_end AND (end > query_start OR end IS NULL)`;`UNKNOWN_INTERVAL` 只能参与低置信 partial overlap,不得触发 direct contradiction。
...
**业务幂等默认值(v15.1 收敛)**:
不维护全局大表。P0 只冻结两个会产生用户可见重复副作用的默认键:`commitment.fire = (commitment_id, trigger_id, fire_bucket_24h)`,`statement.recalled = (query_id, query_window_2s)`。其他事件默认使用一次性业务键 `(event_type, aggregate_id, version_or_sequence)`;Subscriber 若需要覆盖,必须在 Bus 配置中显式注册。
...
巩固结果用 `derived_from` 指回直接输入 Statement;完整派生链由递归展开计算。原始证据按 Drawer retention_mode 保留、脱敏或加密擦除,不再用"永不删除"作为统一规则。
...
- `testing.mark_evidence_erased(drawer_ref)`:把测试 DrawerRecord 标为 `crypto_erasure` 并同步 evidence erased 标记,用于 basic_retrieve 擦除负例;不实现完整合规传播。
...
| 子里程碑 | 内容 | 依赖 |
|---|---|---|
| M0.1 Schema | Pydantic 实现 + JSON-Schema 导出,含 `tenant_id`、chunk 级 `SourceSpanRef` 与 P0 推导式 `TemporalAnchor` 序列化 | 无 |
| M0.2 持久化 | SQLite CRUD + outbox 表 + consumer_checkpoint + 最小 `PipelineRun` 表;`ExtractionAttempt` 作为 extraction run item 表;不实现 step graph | M0.1 |
...
| M0.5 冲突 | ConflictProbe + `canonical_conflict_key` + `canonical_object_hash` 主表列 + `SUPERSEDES` 边 | M0.4 |
...
| m1 | Minor | SourceSpan P0 最小化:只要求 drawer_ref/chunk_index/observed_at/source_hash,offset/segment_map/segment_id 延后到 P2+ | §3.2、§3.8、§5.1 |
| m2 | Minor | P0 非交付项补完整 ActionPolicyGraph、PipelineStepContract、细粒度 segment_map 与完整 RetrievalScopePlan runtime | §16.1 |

### 17.15 v15 → v15.1 修复(本版本)
...
| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| B1 | Blocker | `evidence` 硬约束按 provenance 分级:直接抽取必须有 Drawer evidence/source span,派生 Statement 可无直接 evidence 但必须有 `derived_from`;总规则为 `evidence` 与 `derived_from` 至少其一非空且组合合法 | §3.2、§5.1 |
