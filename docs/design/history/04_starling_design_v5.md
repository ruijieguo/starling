# Starling Memory v5:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v5,基于 v4 经资深架构师重审(`06_v4_arch_review.md`)后,对运行时架构层做系统性重写,修复 2 个 Blocker + 8 个 Major + 5 个 Minor 级问题。**数据模型(Statement schema、社会心智本体、ToM API)保持与 v4 一致;运行时层(状态机、Bus 契约、分层抽象)经实质重写**。
>
...
**Starling v5 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
...
> **设计原则(v5 重写,确立三层抽象)**:
>
> 1. **逻辑层**(本节 §3):所有可写入的"陈述"都是 `Statement` 或其子类(灵感:cognee DataPoint Annotated 子类化);所有"容器型实体"(Persona / CommonGround / KnowledgeFrontier)继承 `Container`,装载 Statement 但本身不是 Statement。**Statement vs Container 是基础二分**(v5 新增,解 v4 自相矛盾)。
...
`EntityRef` 是 `Entity.id` 的引用。Statement.subject/object 接受 `CognizerRef | EntityRef | StatementRef`(StatementRef 用于二阶 ToM 嵌套,§3.2)。Entity 注册由 Cognizer Hub 的 NER 流程 + alias 归一负责(§8.1 discover 阶段),与 Cognizer 共享 alias 算法但**Entity 无 persona / KnowledgeFrontier / trust_priors**(它不是认知主体)。
...
### 3.4 ConsolidationState 状态机(v5 重画,REPLAYING 拆为 2 子态)

> **v5 修复**:v4 状态图缺 4 条关键边(尤其 `CONSOLIDATED → REPLAYING`,§11 整章核心),且 REPLAYING 单态承载两种意图(巩固 vs 再巩固)。v5 拆为 `REPLAYING_CONSOLIDATING` 与 `REPLAYING_RECONSOLIDATING` 两子态,补全所有迁移边。
...
```
EpisodicView   = WHERE event_time IS NOT NULL OR participants IS NOT EMPTY
...
任何 Statement 的 `evidence` 必须指向 Drawer 中的 verbatim 片段。Drawer **物理上 append-only**,即使 LLM 抽取错误,原档永远在(借鉴 mempalace);**生命周期独立于 Statement** —— Statement FORGOTTEN 后 Drawer 仍在(为合规审计保底)。多 Cognizer 可共享同一 DrawerRecord(参考 Letta Archive 模型,§16.2)。
...
### 3.10 Statement 写入路径分类(v5 新增,解 Blocker B2)
...
**所有 Statement 按生成来源分三类**,每类 Bus 处理规则不同:

| 来源 | `provenance` | 入态 | 是否 emit `statement.written` | 是否被 Replay 重新采样 |
|---|---|---|---|---|
| **用户输入(直接观察)** | `user_input` | VOLATILE | ✅ | ✅(默认) |
| **Replay 派生(compress/abstract 等)** | `replay_derived` | **直接 CONSOLIDATED**(跳过 VOLATILE) | ❌ emit `statement.derived` 替代 | ❌(避免循环) |
| **ToM 推断(perspective_take 等运行时算子产出)** | `tom_inferred` | VOLATILE | ✅ 但带 `is_inferred=True` 标记 | ✅(允许),但 Replay 优先级降权 |

**Statement schema 新增字段**(v5):

```python
class Statement(BaseEntity):
    # ... v4 全部字段(包括既有的 derived_from: list[StatementRef] 推断链)...
    provenance: Literal["user_input", "replay_derived", "tom_inferred"]   # v5 新增
    derivation_chain: list[StatementRef]    # v5 新增:仅当 provenance="replay_derived" 时,指向被压缩/抽象的源 Statement 集
                                            # 与 §3.2 既有的 derived_from(一般推断链)语义并存:derivation_chain 专指 Replay 派生
```

**Bus 处理规则**(v5,与 §5.3 BusEvent 表对齐):

```python
def Bus.write(stmt: Statement):
    Validator.check(stmt)                # XML/Schema/隐私边界
    ConflictProbe.scan(stmt)             # 冲突探针
    
    if stmt.provenance == "replay_derived":
        # 绕过 VOLATILE,直接进 CONSOLIDATED
        partition = Neocortex
        partition.upsert(stmt, state=CONSOLIDATED)
        emit("statement.derived", stmt)  # 不发 .written,避免 Replay 重入
    else:
        partition = Hippocampus
        partition.upsert(stmt, state=VOLATILE)
        emit("statement.written", stmt)
```

**Replay 订阅规则**(v5):
- Replay 仅订阅 `statement.written`,**不订阅 `statement.derived`**
- 因此 Replay 自身产出的 `replay_derived` Statement 不会触发新一轮 Replay,断开 §10.3 → §5 → §10 循环
...
### 5.3 BusEvent(v5 补全 8 个事件 + producer/consumer 完整对照)
...
| `statement.derived` (v5 新增) | Bus(写完 Neocortex 后) | ToM(更新 Persona) | Replay 派生 stmt 直接进 CONSOLIDATED |
...
| `statement.superseded` | Reconsolidation | ToM(更新 CommonGround) | REPLAYING_RECONSOLIDATING 产 supersedes 链 |
...
**契约**:
1. **同 key 合并**:同一 `(subject, predicate, holder)` 在窗口 W(默认 10s)内触发的同类事件,合并为一个事件(取最后一次 payload),避免冲突反复仲裁。
2. **递归深度限制**:每个事件携带 `causation_chain: list[event_id]`,链长 ≤ 3。超过则 emit `system.runaway` 告警并停止派生。
3. **幂等键**:每事件 `idempotency_key = hash(producer, target_id, type, window_bucket)`。Bus 维护 N 分钟 LRU,重复键直接丢弃。
...
> **v5 重写**:v4 把 Drawer 误归入 §6.1 Hippocampus 内部;v5 把 Drawer 提升为与 Hippocampus / Neocortex 平级的全局子系统。

### 6.0 Drawer:全局原档子系统(v5 新增独立小节)

Drawer 是 Starling 的最底层证据保底,**与 Hippocampus / Neocortex 平级**,不属于任何记忆子系统:

```
┌─────────────────────────────────────────────────────────┐
│  Drawer  ── 全局 BlobStore(append-only, immutable)      │
│   • 任何 Statement.evidence 必须指向其中片段            │
│   • 生命周期独立于 Statement                            │
│   • Statement FORGOTTEN 后 Drawer 仍在(合规审计保底) │
...
**物理底座**:S3 / 本地 fs / Letta archival / memU Rust blob 层(§16.2)。
...
| 动作 | 输入 | 输出 | provenance | 状态迁移 | 是否触发再 Replay |
|---|---|---|---|---|---|
| `compress` | 多条相似 EpisodicEvent | 1 条更高层语义 Statement(Drawer 指针保留) | `replay_derived` | 输入:VOLATILE→CONSOLIDATED;输出:直接 CONSOLIDATED | ❌(v5 默认 non-propagate;借鉴 memU v1.5.0 #386,显式 propagate=True 时才级联) |
| `abstract` | 多 holder 的同 predicate | 1 条 Persona trait 更新候选 | `replay_derived` | 输入留态;Persona 容器更新 | ❌ |
...
| `induce_norm` | 多次 PREFERS/COMMITS 同模式 | 1 条 Norm 候选,人工/规则确认入库 | `replay_derived` | 候选:VOLATILE 待人工 confirm 后 → CONSOLIDATED | ❌ |
| `forge_skill` | 多次成功 Case 集群 | 1 条 Skill 候选(EverOS AgentSkill 风格) | `replay_derived` | 同 induce_norm | ❌ |
| `decay` | 低 salience 长未召回 | confidence 衰减,达阈值 → state 迁移 | — | CONSOLIDATED → ARCHIVED(emit `statement.archived`) | — |
| `purge_compliance` | 用户撤回 / 法务事件 | state→FORGOTTEN + 传播到 derived 链(Drawer 仍留) | — | ARCHIVED 或 CONSOLIDATED → FORGOTTEN(emit `statement.forgotten`) | — |

**关键不变量(v5)**:所有 `replay_derived` 输出**经 Bus.write 但带 provenance 标记**,Bus 把它写入 Neocortex 直接为 CONSOLIDATED,emit `statement.derived`(非 `statement.written`),Replay Scheduler 不订阅此事件 —— 闭环断开,无重入。

巩固结果带 `derivation_chain` 指回原 Statement,**不删除原始证据**。
...
**被 forget 不删 Drawer**,只逻辑下降。Starling 永远可审计。
...
def reconsolidate(stmt, pending_evidence):
    ...
    if supports(aggregated, stmt):
        ...
        stmt.access_count += 1
        stmt.consolidation_state = CONSOLIDATED         # 回到稳定态
        bus.emit("statement.consolidated", stmt)
        ...
        if mild:
            ...
            new_version.provenance = "tom_inferred"      # v5 新增
            bus.write(new_version)                        # 经 Bus 走完整 Validator
            stmt.consolidation_state = ARCHIVED          # 不删
            bus.emit("statement.archived", stmt)
            bus.emit("statement.superseded", {"old": stmt, "new": new_version})
...
### 12.2 Commitment 五态机(v5:状态变更全经 Bus 派发事件)
...
- `FULFILLED` / `WITHDRAWN` 是终态,不可再变;但其影响(trust_priors 调整)由 commitment.fulfilled 事件异步触发

每条 Commitment 由后台 PolicyEngine 持续监听 Trigger,命中时**经 Bus emit `commitment.fire`**(v5:PolicyEngine 是 Bus 的 publisher 之一,所有 Trigger 命中走 `Bus.emit("commitment.fire", ...)`,不绕过 Bus),Working Set 注入 `pending_commitments` block。

**PolicyEngine 与 Bus 的关系(v5 新增,解 Major M8)**:
```
PolicyEngine
  ├── 订阅:commitment.fulfilled / commitment.broken / commitment.renegotiated
  │   (用于 trust_priors 调整、Persona Replay 候选)
...
- Identity ORM(`identifier_key + identity_type + properties`) → Cognizer 的迁移起点(Letta Identity 已有主体注册,Starling 在其上加 `aliases / canonical_name / trust_priors`)
...
- Episode 中心架构 → Starling EpisodicEvent 逐一映射,但需注意 Graphiti 的 `reference_time` 和 `episodes` 列表字段(Edge 上显式把 episode 作为"事实证据来源")
