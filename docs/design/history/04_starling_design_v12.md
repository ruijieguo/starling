# Starling Memory v12:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v12,基于 v10/v11 与 9 个开源代码库的进一步源码级复审(mem0 / Letta / cognee / Graphiti / MemOS / EverOS / memU / mempalace / claude-mem),补齐 source adapter 转换声明、自污染防护、底座命令边界租户隔离、长任务 step revision、索引修复安全阈、检索充分性回执与 trace retention。v12 不改变核心认知本体,只把源码中反复验证有效的线上防线写成 Starling 的可执行契约。
>
...
**Starling v12 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
