...
EngramStore 是与 [Hippocampus](v19_06_hippocampus.md) / [Neocortex](v19_07_neocortex.md) 平级的全局证据子系统，存储 verbatim 原档，按 `retention_mode` 管理内容生命周期；审计元数据 append-only，内容是否可恢复由加密策略决定。它是 `Statement.evidence` 指向的物理目标，所有写入须经 [Bus](v19_05_bus.md)`.append_evidence`，不允许组件直接读写底层 blob。它不做：派生抽取、检索排序、业务逻辑路由。
...
合规引擎（[Governance](v19_05_governance.md)）持有状态迁移授权；EngramStore 本身不自主触发迁移，仅执行指令并追加 `audit_trail`。
...
多 Cognizer 可引用同一条 Engram。任一主体触发 `crypto_erasure` 时，"最严格访问者 wins"规则生效：共享记录须整体擦除或拆分引用后分别擦除，不得向其余 holder 继续暴露原文。refcount 维护由 EngramStore 内部执行；[Bus](v19_05_bus.md) 不感知具体引用计数，仅转发 Governance 授权令牌。

**相关子系统**

- [Bus](v19_05_bus.md)：`append_evidence` 唯一写入入口；`evidence.appended` 事件发布方。
- [Governance](v19_05_governance.md)：持有 retention 状态迁移授权；下发 crypto_erasure 指令。
- [Hippocampus](v19_06_hippocampus.md) / [Neocortex](v19_07_neocortex.md)：平级存储子系统，均通过 `Statement.evidence` 引用 EngramStore 中片段。
- [Retrieval](v19_13_retrieval.md)：通过 `SourceSpanRef` 反查 EngramStore 获取 verbatim 原档，拼装 Context Pack。
- [Substrate](v19_04_substrate.md)：物理底座（S3 / 本地 fs / Letta archival / memU Rust blob 层）。
