# Starling Memory v16:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v16,以 v15.1 为基线做 P0 编码前最后一轮契约收口。v16 不新增认知能力,只补齐 Value 标准化、hash 版本、跨 tenant 派生、业务幂等默认键、testing helper 识别与派生深度缓存等会影响 M0.1/M0.2 编码的规则,并继续遵守 v15 的简单性原则:能用主表、固定路径、单一前驱字段和少量索引解决的问题,不引入第二套运行时框架。
>
...
| 函数 | 输入 | 规则 |
|---|---|---|
| `canonicalize_object` | Value / EntityRef / CognizerRef / StatementRef | StatementRef 只取一阶展开 `(holder, subject, predicate, polarity)` 与 inner id hash,不递归展开;EntityRef/CognizerRef 取 canonical_name + id;Value 按 §3.2 Value 表标准化;list/dict 不允许兜底 hash |
...
|---|---|---|---|
| `evidence.appended` (v6 新增) | Bus.append_evidence | Extractor / Cognizer Hub / Audit | Engram 事务提交 |
| `evidence.redacted` (v6 新增) | Compliance Engine | Retrieval index / Audit | Engram 转 redacted_retain |
| `evidence.erased` (v6 新增) | Compliance Engine / KMS | Retrieval index / Audit | Engram crypto_erasure 完成 |
...
| `container.dimension_invalid` (v15.1 新增) | Bus.rebuild_container | Audit / Ops | Container rebuild 请求使用未声明 dimension key |
...
| `extraction.noop` (v11 新增) | Extractor | Audit / PipelineRun | Engram 已处理但无新 Statement |
...
| 阶段 | 时间 | 目标 | 核心交付 | 验证方式 |
|---|---|---|---|---|
| **P0 Spike** | 2 周 | 可执行契约冻结 + 最小闭环 | Statement + Cognizer + 六态 ConsolidationState + StatementProvenance + ReviewStatus + EngramRetentionMode 的 Pydantic 实现与 JSON-Schema 导出;SQLite 单底座 CRUD;outbox 表;`ProfileCapability` preflight;最小 `PipelineRun` 账本 + `ExtractionAttempt` item 明细;`SourceKind/IngestPolicy` 与 source adapter metadata;P0 chunk 级 `SourceSpanRef`(`engram_ref/chunk_index/observed_at/source_hash`)与序列化推导的 `TemporalAnchor`;一阶 Statement 抽取 prompt(XML strict);`basic_retrieve(holder=self, intent=FACT_LOOKUP)` 主表直查 + 最小 `RetrievalReceipt` | 单元测试 + 50 条手工标注对话样本回归 + outbox 投递/幂等测试 + §14.1 最小变体 smoke test: Alice 宣布 Bob 不再负责 auth,testing helper 将关键 Statement 标为 CONSOLIDATED 后,检索返回 Carol 接手,并能查看 extraction attempt、pipeline run 与 retrieval receipt |
