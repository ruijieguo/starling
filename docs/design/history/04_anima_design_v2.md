# Anima Memory v2:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v2,在 v1(`04_anima_design.md`)基础上吸收 Polis 提案(`03_design_proposal.md`)的优点,查漏补缺,保持完整性与一致性。
>
> **v2 相对 v1 的主要增量**(详见 §17 变更记录):
> 1. 时间维度补到 5 种(增 `inferred_at`)
> 2. 显式 `consolidation_state` 状态机
> 3. `polarity / modality` 两轴分离
> 4. Cognizer 增 `trust_priors / aliases / canonical_name`
> 5. Reconsolidation(Nader 2000)显式建模为 §11
> 6. 模式分离用"反相似偏移 + MAY_OVERLAP_WITH 软边"
> 7. 7 个 Mentalizing Retrieval Primitives 高阶 API
> 8. Context Pack Builder 心智摘要输出(带语用标注)
> 9. 五类记忆从"分子区"改为"视图"(更优雅)
> 10. 新增 §18 关键设计取舍 与 §19 风险与开放问题
> 11. 复用资产对照表(§20):谁干什么 + 加了什么
> 12. Norm.enforcement_history / Commitment.RENEGOTIATED 状态
>
> **保留并强化的 v1 优点**:
> - Trigger 类型系统(`time/event/state/compound`)
> - Abstention 显式判定
> - SubstrateAdapter 5 profiles
> - 二阶 ToM 物理 `nesting_depth` 实现
> - Fiske 关系四类标签
> - 与 8 项目能力对照表
> - "为什么更懂人"的认知科学解释
...
7. **视角化检索 + 心智摘要输出**:Retrieval Planner 9 Intent × Perspective Filter × Affect Reranker × Context Pack Builder。
...
LLM 在线计算每条话语的 `surprise`(下一句对上一段的预测对数似然反向),跨阈值即切出 EpisodicEvent。
...
- **Persona(慢通道)**:traits / values / preferences / competencies,**仅由 Replay 周期更新,单次会话不触动**;
...
### 9.4 Mentalizing Retrieval Primitives(7 个高阶 API,v2 吸收 Polis)
...
**这 7 个 API 把 LLM 从"每次重新模拟"中解放出来,系统负责维护可查询的认知状态**。
...
| `reconcile` | 冲突 Statement 集 | supersedes 链 + 历史信念归档(转 Reconsolidation §11) |
...
任意 CONSOLIDATED Statement 一旦满足以下之一,进入短暂 `REPLAYING` 可塑窗口(默认 5 分钟):
...
def reconsolidate(stmt, new_evidence_or_input):
    ...
    schedule_close_plastic_window(stmt, timeout=5min)
```
...
7. abstain:  if 总分 < 阈值 → 输出"无可靠记忆",避免编造
```
...
|---|---|---|---|
| 长期多会话回忆 | LongMemEval 五类问题(单跳/多跳/时间/更新/缺失) | 分类 accuracy | Statement 抽取 + holder 归属 + Replay 巩固 |
| 时序推理 | LoCoMo 时间线 / 自建时间锚点 QA | 时间顺序正确率、valid_from/to 边界准确率 | §3 五种时间字段 + §12 time-aware planner |
| 模式补全质量 | 自建:给 partial cue(如"那次和 Alice 去的咖啡馆"),评估回忆完整度 | Recall@k of related episode subgraph | CA3-style PPR 图游走(§12.2) |
| 模式分离能力 | 注入高度相似但不同的两个 episode,检查是否混淆 | 混淆率(越低越好) | 写入反相似性偏移 + MAY_OVERLAP_WITH 软边(§5.2) |
| 巩固一致性 | 注入冲突信息,检验多视角保留 | 冲突保留率 + supersedes 链完整率 | Reconsolidation 不覆盖 + CONFLICTS_WITH 边(§7.3) |
| 自适应遗忘 | 注入大量噪声 statement,观察查询效率 | Latency vs total memory size 曲线,关键记忆保留率 | MemoryBank + Anderson 公式 + 降级不删除(§10.4) |
| 情感记忆偏置 | 配对高/低情感事件,间隔测试 | 高情感事件回忆优势比(模拟人类效应) | AffectVector.salience 调制写入强度与重放优先级(§10.2) |
| 再巩固保真度 | 注入需修正的旧信念,检查修正后旧版本可追溯性 | 旧版本可检索率、新版本引用率 | RENEGOTIATED 状态机 + supersedes 链保留历史(§7.3) |

### 15.2 社会心智评测

| 评测项 | 方法 / 数据集 | 核心指标 | Anima 对应机制 |
|---|---|---|---|
| 一阶 ToM(信念归因) | ToMBench / SocialIQA | 信念归因 accuracy | holder=other 子图 + KnowledgeFrontier mask(§9.2) |
| 二阶 ToM(嵌套信念) | FANToM / Hi-ToM / EnigmaToM | 嵌套信念 accuracy | 嵌套 Statement(nesting_depth=2)(§9.2) |
| 信息不对称(FANToM) | FANToM 标准集 | 不对称检测 F1 | perceived_by + presence_log(§9.2) |
| 多视角切换(SoMi) | SoMi-ToM 1225 题(第一/第三人称双视角) | viewer-conditioned QA accuracy | perspective_take 算子(§9.3) + holder-aware 检索 |
| 共同知识建立 | 自建:多主体对话中追踪 SharedGround | common ground inference F1 | CommonGround pool + explicit grounding acts(§9.4) |
| 承诺履行 | 自建:对话中含承诺,检验 N 步后触发 | Commitment detection + reminder timeliness | Prospective Loop + Trigger 类型系统(§11) |
| 信念修正传播 | 注入"其实...是错的"修正,看关联信念是否联动更新 | 旧信念使用率(应趋零) + 旧版本可追溯性 | A-Mem 风格的演化传播 + supersedes 链(§7.3) |
| 视角泄露防护 | 自建:检查是否向未在场者泄露信息 | 视角泄露率(应趋零) | EnigmaToM 风格 iterative masking + perspective filter(§12.3) |
| 二阶 ToM 主动提示 | 自建:"你应该提醒 Bob,他可能还不知道..." | 主动提示 precision/recall | 二阶 QueryIntent(META_BELIEF) + suspected_diverge 标记(§13.3) |
...
## 17. v2 变更记录(相对 v1 `04_anima_design.md`)

| 变更 | v1 | v2 | 理由 |
|---|---|---|---|
| **公理** | 3 条(归属+双系统+目标重构) | 3 条,措辞强化 | 不变 |
| **Statement 状态机** | VOLATILE→REPLAYING→CONSOLIDATED→ARCHIVED→FORGOTTEN | 同,新增 RENEGOTIATED 为 CONSOLIDATED 的可选子状态 | Nader 2000 再巩固:检索触发的不稳定窗口 |
...
| **Reconsolidation** | 提到概念 | **完整状态机 + enforcement_history + 不覆盖只追加** | 这是 2025 研究确认的核心差异点 |
...
7. **再巩固窗口的边界控制**:Reconsolidation 允许修改已巩固记忆,但窗口太宽会导致记忆不稳定,太窄则失去修正机会。缓解:Nader 2000 的实验参数作为初始默认(检索后 ~6h 窗口);production 中基于更新频率自适应调整。
