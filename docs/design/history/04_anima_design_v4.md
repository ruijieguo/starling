# Anima Memory v4:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v4,基于 v3 经 9 仓库源码重审 + 2026 前沿论文检索后,修正 10 项关键发现并纳入 8 篇 2026 新文献。设计本体未变,准确性与时鲜度显著提升。
>
> **v4 相对 v3 的修复**(详见 §17.3 v4 修复记录):
> 1. **修正 graphiti 时间字段**:valid_at/invalid_at/expired_at 三时间(非两时间),新增 SagaNode 增量摘要节点
> 2. **补全 EverOS HyperMem(2026 ACL)** :三级 hypergraph + coarse-to-fine RRF 检索
> 3. **修正 mem0 隔离维度**:actor_id 作为第 4 维查询维度(Cognizer 四维隔离)
> 4. **修正 cognee improve()** :feedback_weight alpha 已实装(非 TODO)
> 5. **修正 Letta 迁移描述**:GitEnabledBlockManager 真 git + Identity ORM + Archive
> 6. **删除 claude-mem cynical deletion 错误对照**:cynical deletion 实为代码风格运动(非数据删除)
> 7. **补全 mempalace Hall detection + corpus origin**:§3.1 + §7.1 吸收
> 8. **更新 MemOS 描述**:TreeTextMemory + 9 个真实 scheduler 模块
> 9. **更新 memU 描述**:Rust core 重写 + non-propagate 开关
> 10. **纳入 2026 前沿论文**:8 篇新文献入 §21 + A-ToM 入 §9.6 (Adaptive ToM Order)
>
> **沿袭 v3 的设计本体**:
...
> - 与 9 项目能力对照表 + "为什么更懂人"的认知科学解释

---

## 0. 摘要(One Page)

**Anima v4 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
...
**v4 增——hyperedge 方向参照**:EverOS HyperMem (2026 ACL) 使用加权 hyperedge $w \in [0,1]$ 表达"多事实共属同一 episode"这种 N 元关系,而非逐对各建一条边。Anima 当前边类型表是二元的(Statement-to-Statement),N 元场景(如"Alice、Bob、Carol 三人在场共识了 X")需靠 CommonGround pool 间接承受。当共现场景常态化时,可引入 hyperedge 作为 `SHARED_GROUND` 的物理实现替代。见 §13.5 + §20。
...
**对照**:9 个开源项目都把存储绑死。Anima 把"存储绑定"作为部署选项而非架构决策。
...
**对照**:claude-mem 的 4 阶段 hook(v12.4.4 移除 SessionEnd)是 session 级,Anima 是 statement 级。
...
**为什么需要 EpisodicEvent + episodic_link**:Episodic Knowledge Binding (OpenReview 2026) 系统刻画了 LLM 跨时间绑定 episodes 的根本失败模式 —— LLM 擅长学习孤立事实,但无法跨时间绑定相关 episodes。这是 parametric memory 的根本缺口,也是 Anima §3.6 EpisodicEvent + §3.7 SHARED_GROUND/OBSERVED_BY/MAY_OVERLAP_WITH 边类型的设计动机。
...
**主流开源系统中无一显式注入** `interlocutor_persona / common_ground / pending_commitments` 这三块 —— 这是 Anima 工作记忆的差异点。
...
**v4 增——mempalace Hall detection 对照**:mempalace v3.3.0 引入 7 个 Hall (`emotions / technical / family / memory / identity / consciousness / creative`),基于内容情感/主题做存储分区。Anima 的 5 子区是按记忆类型(语义/技能/规范/画像/共识)划分,两者维度不同但可组合 —— Hall 可作为 Neocortex 子区内的二级分区策略。见 `_research/04_repos_resurvey_2026.md` §mempalace。
...
**为什么必须分离**:mem0/memU 等系统把"用户偏好喝咖啡"这种长期 trait 与"用户当前在生气"这种瞬时 belief 写到同一个 profile 里 —— 一次会话就能颠覆长期画像。Anima 的双通道在 schema 层禁止这种污染。
...
A-ToM (arxiv 2603.16264, AAAI 2026) 提出首个自适应 ToM agent,可实时估计 partner 的 ToM order,解决"固定 ToM order 在不同 partner/场景下适配性差"的问题。Anima 的 ToM Engine 默认追踪深度 ≤ 2(§9.1),深度 3 仅显式触发。A-ToM 的发现建议增加 `ToMDepthEstimator` 模块:
...
策略:当估计 partner 为一阶时,Anima 仅需维护 `nesting_depth ≤ 1` 的 statement;当估计为二阶时,允许 `nesting_depth = 2`。这同时为 §13.2 `META_BELIEF` Intent 提供深度上限指导,避免对低 ToM-order partner 做过度二阶推理。此模块排入 P4。
...
| 动作 | 输入 | 输出 |
|---|---|---|
| `compress` | 多条相似 EpisodicEvent | 1 条更高层语义 Statement(Drawer 指针保留)。默认 `non-propagate=True`(不触发下游摘要连锁更新,避免延迟爆炸;借鉴 memU v1.5.0 #386 non-propagate patch 开关),显式 `compress(propagate=True)` 时触发级联 |
...
**被 forget 不删 Drawer**,只逻辑下降。Anima 永远可审计。

> **v4 勘误**:v3 在此处写"claude-mem 的 cynical deletion 真删,Anima 永远可审计" —— **此为范畴错误**。claude-mem "cynical deletion" 实为代码风格运动:删除 defenders(orphan cleanup/多余 liveness probe)与 tolerators(silent JSON drops/passthrough Zod schemas),用 fail-fast 与 strict boundary 替代防御性 try/catch。它是代码质量策略,不是数据删除策略。v4 删除此错误对照。(ref: claude-mem ANTI-PATTERN-TODO.md 301 issues/289 fixed, CHANGELOG v12.4.7)
...
> 主流开源系统全部缺失,这是 Anima 与 RMM 之外少数把"前瞻"做成 first-class 的设计。
...
**v4 增——Coarse-to-fine RRF 方向**:EverOS HyperMem (2026 ACL) 使用三级 coarse-to-fine 检索: Stage1 Topic → Stage2 Episode → Stage3 Fact,以 RRF $\sum 1/(k+\mathrm{rank}_m)$ 融合 BM25+dense,LoCoMo 92.73% 超越 MemOS 16.93 个点。Anima 当前 reranker 是单级加权乘子,可升级为 HyperMem 风格的三级 RRF 融合,尤其适合"先从大量 candidate 中筛 Topic,再精排 Episode→Fact"的大记忆体场景。进 P4 检索规划器时选配。

**v4 增——时间距离惩罚项**:cognee `temporal_retriever.py` 用 `triplet_distance_penalty`(默认 6.5)在距离查询时间窗的 triplet 上降权。Anima 已有 `recency_factor(c)`,但无显式的"事件时间距离查询锚点"的绝对值惩罚。v4 在 rerank 公式中新增 `temporal_distance_penalty` 项。
...
[
  ...
  S4: holder=Alice, subject=Bob,   pred=responsible_for, obj=auth, mod=BELIEVES, pol=NEG, valid_to=NOW
   ...
   ↓ Reconsolidation Engine:旧 Bob 责任 statement 进入 REPLAYING 可塑窗口(高频更新场景缩短至 5min,见 §11.1)
...
**关键差异点**:现有开源系统没有任何一个能在没有用户手动 query 的情况下"周三早上自己想起来这件事"。Anima 通过 Trigger + PolicyEngine 把"主动惦记"做成了运行时一等公民 —— 不依赖 cron job 或外部 reminder,而是从 Statement 数据本身派生触发条件。
...
- Identity ORM(`identifier_key + identity_type + properties`) → Cognizer 的迁移起点(Letta Identity 已有主体注册,Anima 在其上加 `aliases / canonical_name / trust_priors`)
...
**从 Graphiti / Zep 迁入**:
- Graphiti 字段是 `valid_at / invalid_at / expired_at`(三时间,非两时间,v4 勘误):`valid_at`(事实生效)、`invalid_at`(事实停止)、`expired_at`(被新事实推翻)。Anima 使用 `valid_from / valid_to` 映射到 `valid_at / invalid_at`,并复用 `expired_at` 机制作为 supersedes 链的时间戳底座
- SagaNode(v4 新增引用):Graphiti 的增量摘要节点(`last_summarized_at`,只读取新 episode 续写摘要) → Anima Replay Scheduler `compress` 动作的参考实现
- 增加 holder 维度(Anima 把全局图族化,Graphiti 本身无 holder)
- Episode 中心架构 → Anima EpisodicEvent 逐一映射,但需注意 Graphiti 的 `reference_time` 和 `episodes` 列表字段(Edge 上显式把 episode 作为"事实证据来源")
- MinHash + Shannon entropy + Jaccard ≥ 0.9 实体名去重 → Anima Cognizer alias 归一(§3.1)的算法候选
- `resolve_edge_contradictions`(新边设旧边 `expired_at`,节点就地不删) → 与 Anima supersedes 链同型,可复用 cypher
...
| **claude-mem** | 4 阶段 Hook 生命周期(SessionStart/UserPromptSubmit/PostToolUse/Summary,v12.4.4 移除 SessionEnd);SQL 主 + 向量副降级;严格 XML 输出契约;Defenders/Tolerators 分类(代码风格,非数据删除,v4 勘误) | Hook 接 Anima 的写入端(抽取→冲突检测→Replay 调度);Observation 升级为 Statement;Defenders/Tolerators → §5 Validator + §13.7 Abstention Gate 工程纪律引用;`<private>` tag 边缘剥离 → §3.2 `visibility / retention_policy` 实现参考 |
| **graphiti / Zep**(v4 新增独立行) | 三时间字段 `valid_at/invalid_at/expired_at`;**SagaNode** 增量摘要节点(`last_summarized_at`);MinHash + Shannon entropy + Jaccard 实体名去重;`resolve_edge_contradictions`(新边设旧边 expired_at,节点不删);`DateFilter` CNF 时间过滤;Cross-encoder rerank | 三时间复用为 Anima 四时间制(+ inferred_at);SagaNode → Replay `compress` 动作的参考实现;dedup_helpers 算法 → Cognizer alias 归一(§3.1);resolve_edge_contradictions cypher → supersedes 链物理实现;CNF DateFilter → Retrieval Planner 时间锚点查询(§13);加 holder 维度后,group_id 之外叠 holder 标签做子图族 |

> Anima 要造的不是第 10 个开源记忆库,而是它们之上的认知层。
...
- **Human-Like Lifelong Memory** (arxiv 2603.29023, ICLR 2026 MemAgent Workshop) —— CLS 架构 + CBT belief hierarchy,Anima CLS 路线的学术背书
...
| 能力 | mem0 | Letta | cognee | MemOS | EverOS | memU | mempalace | claude-mem | graphiti | **Anima** |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| holder 归属 | ❌ | ❌ | ❌ | ❌ | 🟡 | ❌ | 🟡 | ❌ | ❌ | ✅ |
