# Starling Memory v17:多主体社会心智 + 类脑动力学的智能体记忆系统设计方案

> **版本说明**:本文为 v17,以 v16 为基线做 Section 1+2 plan-eng-review 后的契约修复。v17 是 P0 编码起点(不是 v16),将 14 项契约决定 + 3 项策略决定全部写入文档,代码与文档同步。继续遵守 v15 的简单性原则与 v16 的 P0 编码契约边界。
>
> **v17 相对 v16 的修复**(详见 §17.17 v16 → v17 修复记录):
>
> *Major 级*:
> 1. **VOLATILE 振荡 + TTL 双防线** — 加 MAX_CONSOLIDATION_ATTEMPTS=5 强制升级 + T_max_volatile=7 天兜底(§3.4/§10.2)
> 2. **Commitment 循环防护** — BROKEN 计数 ≥3 自动 WITHDRAWN + RENEGOTIATED 链长 ≤3(§12.2)
> 3. **REPLAYING_CONSOLIDATING fallback timeout** — READY 1h / DEGRADED 4h 自动回退 VOLATILE(§3.4/§5.6)
> 4. **decay 路由经事件总线** — Replay decay 改 emit `statement.decay_candidate`,per-stmt 顺序串行,消除 T5/T8 并发竞态(§3.4/§10.4)
> 5. **severe path 双路径事务保证** — ProfileCapability 加 `cross_partition_transaction`,cloud profile 走 saga 补偿(§4/§11.2)
> 6. **Commitment 反向保护事件** — emit `commitment.active_holding` / `commitment.released`,decay scheduler O(1) 检查(§3.4/§5.3/§12.2)
> 7. **statement.references_existing 闭环 §11.1 触发路径 2** — supporting/contradicting derived_from 引用都进 Reconsolidation(§5.3/§11.1)
> 8. **holder_perspective Extractor 填值规则** — FIRST_PERSON / QUOTED / HEARSAY / INFERRED 默认判定(§5.1)
> 9. **mild correction provenance 不变 + confidence_history** — 轻微反对修改原 Statement,不产新版(§3.10/§11.2)
> 10. **canonicalize_object 对 StatementRef 补 modality + inner_object_hash** — 二阶 ToM 冲突检测准确(§5.2)
> 11. **derived_from 防抖明示** — §5.4 防抖契约显式列 5 种 Reconsolidation 触发路径(§5.4)
> 12. **canonical_object_hash 升级超期告警** — maintenance rebuild >14 天 emit `projection.upgrade_overdue` + DEGRADED(§3.2/§5.2)
> 13. **默认值冻结** — Online N=3 / Idle T=120s / inbox 7天 / window_bucket 24h(§5.4/§10)
> 14. **system.runaway 入 §5.3 主表** — Producer=Bus, Consumer=Ops+Audit(§5.3)
> 15. **§5.3 BusEvent 表加 P 阶段标注列** — P0 实现者明确每个事件何时上线(§5.3)
> 16. **§7.1 子区表 Persona/CommonGround 标 Container 视图** — 与 §3 二分对齐(§7.1)
> 17. **顶部修复记录与 §17 合并** — 仅保留 §17 权威版,顶部仅列摘要(§17.17)
> 18. **新增 3 张 ASCII 流程图** — 写入路径 / Replay 循环 / Reconsolidation severe path(§14.0)
>
> *策略级 (cross-model tension 解决)*:
> - 保持六态机不变(T1):v4→v16 修复链不破坏,SQLite enum 多三个值的 schema 税可接受
> - P0 时间表调整为 6 周(T2):2 周仅能完成 M0.1+M0.2,Section 1+2 新增 19 条 P0 验收用例使 M0.7 工作量翻倍,诚实估算
> - v17 是 P0 编码起点,不是 v16(T3):Section 1+2 14 项决定全部进文档,代码与文档同步
>
> v15.1 → v16 与更早历史保留在 §17 完整变更记录。
>
> **v16 相对 v15.1 的修复**(详见 §17.16 v15.1 → v16 修复记录):
>
> *Blocker 级*:
> 1. **Value 类型与 canonical 化冻结** — 明确 Value 仅支持 bool/int/float/str/datetime;list/dict 拒收,保证 `canonical_object_hash` 可稳定计算(§3.2/§5.2)
> 2. **canonical_object_hash 版本化** — Statement 增 `canonical_object_hash_version`;ConflictProbe 只同版本比较,升级规则走后台 maintenance rebuild(§3.2/§5.2)
> 3. **跨 tenant 派生例外规则** — `org_default/system` 前驱允许继承请求方 tenant 并审计;多 user tenant 进入 review;未声明协议才拒绝(§5.1)
>
> *Major 级*:
> 4. **派生深度 O(1) 缓存** — Statement 增 `derived_depth`,保留 `derived_from` 为唯一前驱字段,但避免 Replay/ToM 每次递归展开(§3.2/§9.2/§10.2)
> 5. **BusEvent 默认业务键可推导** — 每个 BusEvent 声明 `primary_id`;一次性业务键默认从 `event_type + primary_id + target_state/outbox_sequence` 生成(§5.3/§5.4)
> 6. **testing helper fail-closed 识别规则** — testing helper 必须包级隔离并声明 `__starling_testing_only__ = True`;CI 静态扫描 + production preflight 双防线(§4/§16.1)
> 7. **P0 负面用例补 v15.1 新不变量** — tenant/provenance 不可变、跨 tenant 派生、chunk 幂等与 temporal_anchor None 行为纳入验收(§5.1/§16.1)
> 8. **v16 内收口补丁** — hash 升级期双查/保守协议、SUPERSEDES 不累计 `derived_depth`、`idx_statement_id_tenant`、事件 aggregate 默认表、投影 consolidated/superseded 订阅、Replay eligible set 过滤(§3.2/§4.1/§5.2/§5.3/§10.2)
>
> **v15.1 相对 v15 的修复**(详见 §17.15 v15 → v15.1 修复记录):
>
> *Blocker 级*:
> 1. **证据约束按 provenance 分级** — `evidence ≥1` 不再作为所有 Statement 的硬规则;直接抽取必须有 EngramStore evidence/source span,派生 Statement 必须有 `derived_from` 前驱,二者至少一条链路可追溯(§3.2/§5.1)
> 2. **派生链语义单一化** — 删除 `derivation_chain` 持久字段,只保留 `derived_from` 作为直接前驱;evidence/derivation chain 是递归闭包或缓存(§3.2/§3.10/§9.2/§10.3/§11.1)
> 3. **provenance 写入即冻结** — confidence、review、state、access_count 后续变化不得改 provenance;只有 fork/supersedes 新 Statement 才产生新 provenance(§3.10)
> 4. **tenant_id 入主体与陈述 schema** — `Cognizer.tenant_id` 默认 `default`;`Statement.tenant_id` 由 holder 派生并写入后不可变,让 P0 SQLite 查询隔离可执行(§3.1/§3.2/§16.1)
> 5. **P0 TemporalAnchor 简化** — P0 不必持久化独立 TemporalAnchor 字段,可由 `SourceSpanRef.observed_at` 序列化生成;P2 多片段/segment_map 后再独立持久化(§3.2/§5.1/§16.1)
>
> *Major 级*:
> 6. **Container dimension key 冻结但不提前复杂化 CAS** — 声明 Persona/CommonGround/KnowledgeFrontier 合法 dimension keys;P0/P1 仍允许整体 rebuild + 单 version CAS,维度级 CAS 保持 P2+ 升级项(§3.6/§5.5/§16.1)
> 7. **ConflictProbe P0 主表索引契约** — `canonical_object_hash` 写入时计算,主表提供 conflict lookup / temporal overlap 两类必要索引;这些不是 Projection Index(§5.2)
> 8. **抽取幂等键 P0 定义** — P0 `normalized_span=chunk_index`,避免依赖 LLM 输出文本;segment-level span 留到 P2(§5.1)
> 9. **causation_chain 传播规则最小化** — Subscriber 调 Bus 时显式传 `causation_parent`(可为 None);Bus 拼接链并在发布时执行 ≤3 检查(§5.4)
> 10. **P0 验收边界收敛** — P0 smoke test 使用 direct state transition helper 模拟巩固,不引入生产 admin API 或完整 Replay/Compliance/Reconsolidation(§16.1)
>
> 更早版本的完整修复历史保留在 §17 变更记录;顶部只保留与 P0 编码直接相关的 v16 / v15.1 摘要,避免实施团队进入正文前先读完整 changelog。

---

## 0. 摘要(One Page)

**Starling v15 解决一个问题**:让 LLM Agent 像人一样,**对每个交互对象都形成一份"持续演化的他者画像 + 我对他的信念 + 我以为他相信什么"**,并且这套画像在系统层面具备类脑的"快写慢洗、优先重放、再巩固、自适应遗忘、显著性调制、前瞻触发"动力学,而不是停在"user_id 隔离 + 向量库 RAG"这种工程层抽象上。
...
**v15 新增的简单性原则**:不是所有正确机制都应该立即实现。`ActionPolicyGraph / PipelineStepContract / RetrievalScopePlan / Projection Index / dimension-level Container CAS` 都有价值,但只有在触发条件出现后才升级。P0/P1 以主表、固定路径、单状态机、最少字段和审计事件为主。
...
Starling 的复杂机制必须有明确触发条件。默认实现遵循:
...
文档中标为 P2+、P4+、P5 的机制不得被 P0 实现团队提前做成通用框架;P0 只证明核心闭环可跑通。
...
│                                                                       │
│  ┌───────────────────────────────────────────────────────────┐        │
│  │ Substrate Adapter:Vector / Graph / KV / EngramStore / Git      │        │
...
> 3. **物理层**(§4 Substrate Adapter):决定实际存储底座(Vector / Graph / KV / EngramStore / KV-cache)。逻辑层与分区层不感知物理底座。
>
...
class Cognizer(BaseEntity):
    ...
    tenant_id: str = "default"              # 单租户部署可固定为 default;多租户写入后不可变
...
**group tenant 规则(v16 新增)**:
- `kind="group"` 的 Cognizer 必须显式声明 `tenant_id`,不得从成员列表隐式推导。
- P0 只支持单 tenant group,默认 `tenant_id="default"`;若发现跨 tenant 成员,写入必须拒绝或进入测试用 `REVIEW_REQUESTED` 分支,不得静默降级为 default。
- P1+ 若启用跨 tenant group,group 自身必须声明 `tenant_id="cross_tenant"` 或显式协议 tenant;以该 group 为 holder 的 Statement 默认 `review_status=REVIEW_REQUESTED`,直到策略或人工确认归属。
...
class Statement(BaseEntity):
    ...
    confidence_history: list[ConfidenceEvent] = []  # v17:mild correction 调整轨迹,append-only
                                                    # ConfidenceEvent = (old_value: float, ts: datetime, evidence_hash: str)
    # —— 时间维度(5 种,v2 补全) —— 
    ...
    source_spans: list[SourceSpanRef]        # 直接抽取的 EngramStore 片段锚;派生仅作 supporting spans
    temporal_anchor: Optional[TemporalAnchor] # P0 可由 source_spans 推导;P2+ 多源时持久化
    derived_from: list[StatementRef]        # 直接前驱;派生 Statement 必填
    derived_depth: int = 0                  # v16:派生链深度缓存;根因=0,写入时由 derived_from 原子计算
    ...
    canonical_object_hash: str              # v15.1:写入时计算,供 ConflictProbe 主表索引使用
    canonical_object_hash_version: str = "v1" # v16:canonicalize_object 规则版本
```

**Value 类型枚举与标准化(v16 新增)**:

`Statement.object` 中的 `Value` 只允许简单标量,避免复合对象让 `canonical_object_hash` 语义漂移。复杂结构必须拆成多条 Statement,或先注册为 Entity 后用 `EntityRef` 引用。

| Value 子类型 | 标准化规则 |
|---|---|
| `bool` | 序列化为 `"true"` / `"false"` |
| `int` | 十进制字符串,不带分组符 |
| `float` | 定点 6 位小数;需要更高精度时必须声明 `high_precision` profile,不得混用默认 hash |
| `str` | Unicode NFC;trim;连续空白折叠为单空格;对有大小写语义的文本执行 lowercase,不改变 CJK 字符 |
| `datetime` | 转 UTC ISO-8601,秒级精度 |
| `list` / `dict` | P0/P1 拒收;列表拆为多条 Statement,结构对象建 Entity 承载 |

若 Value 类型不在上表内,Validator 拒收并 emit `extraction.failed(schema_invalid)`;不得用 JSON dump 兜底。

`evidence` 指向"哪条 Engram 可审计";`source_spans` 指向"这条 Statement 从 Engram 的哪个片段/episode/segment 抽出"。两者都要保留:一个 Engram 可包含多轮消息、多段文档或多 episode,只靠 `chunk_index` 不足以表达 Graphiti `episode_indices`、memU segment resource URL、mem0 Observation Date 这种片段级时间锚。

**证据锚适用范围(v14 收敛)**:
- `provenance=user_input` 且直接由 Engram 抽取的 Statement,必须有 `evidence ≥1`、`source_spans ≥1`;P0 可由 `source_spans[0].observed_at` 自动生成 `temporal_anchor`,P2+ 多源时再持久化独立字段。
- `provenance=tom_inferred` 可以没有直接 evidence/source span,但必须有 `derived_from ≥1`;若只是当次查询 transient context,不得落库也不得 emit `statement.written`。
- `provenance=replay_derived` 可以没有直接 evidence/source span,但必须有 `derived_from ≥1`,直接前驱为被压缩/抽象/归纳的 Statement 集合。
- `provenance=reconsolidation_derived` 可以没有直接 evidence/source span,但必须有 `derived_from ≥1`,直接前驱至少包含被修正旧版 Statement。
- 总校验规则:`evidence` 与 `derived_from` 至少其一非空,且与 provenance 的允许组合一致;派生 Statement 不得把摘要、Container payload 或推断结果伪装成直接原文 span。
...
`derived_from` 是唯一持久化的派生前驱字段,只记录深度 1 的直接前驱。`evidence_chain` 与 `derivation_chain` 都是由 `derived_from` 递归展开得到的可计算闭包,不是 Statement 的新增存储字段。实现可缓存它们,但缓存失效和重建必须由 Statement 主表 + SUPERSEDES/derived_from 边驱动。

`derived_depth` 是唯一允许持久化的派生链缓存,不是第二套前驱字段。写入规则:`derived_from=[]` 时为 0;否则为 `max(parent.derived_depth)+1`,并与 `derived_from` 在同一 Bus.write 事务中原子写入。`SUPERSEDES` 版本链不计入派生深度:`provenance=reconsolidation_derived` 的 severe fork 默认继承被修正旧版的 `derived_depth`,不能因多次纠错而单调累积;若输出实际变成跨多父节点的抽象/摘要,应走 `replay_derived` 而非 reconsolidation fork。若父节点缺失或跨版本不可读,Validator 将新 Statement 标为 `REVIEW_REQUESTED` 并拒绝自动派生级联。

```python
class SourceSpanRef(BaseModel):
    engram_ref: EngramRef
    chunk_index: int
    span_start: Optional[int] = None         # P2+;P0 可为空
    span_end: Optional[int] = None
    segment_id: Optional[str] = None         # P2+;如 resource#segment_3 / episode uuid
    source_role: Optional[Literal["user","assistant","tool","system","document"]]
    source_speaker: Optional[CognizerRef]
    observed_at: datetime                    # 该片段被观察/发生的时间,可不同于 Engram.timestamp
    source_hash: str                         # 片段级 hash,用于重抽/擦除传播

class TemporalAnchor(BaseModel):
    anchor_kind: Literal["source_span","episode","engram_record","message","document","derived_chain","system_now"]
...
- P0 最小实现只要求 `engram_ref/chunk_index/observed_at/source_hash`;`span_start/span_end/segment_id/segment_map` 属于 Adapter conformance 增强,在 P2+ 引入。
- P0 可不暴露独立 `TemporalAnchor` 存储字段,序列化时按 `SourceSpanRef.observed_at` 推导:`anchor_kind="source_span"`、`anchor_time=observed_at`、`resolved_by="metadata"`、`confidence=1.0`、`timezone` 取 Engram metadata(缺失为 UTC)。P2 启用 segment_map 与多片段聚合后,`TemporalAnchor` 必须独立持久化以承载多源不一致。
...
- `FORGOTTEN`:Statement 从热/冷检索路径移除;EngramStore 内容按 `EngramRetentionMode` 执行 redaction 或 crypto erasure,不再默认保留 verbatim。
...
#### §3.4.4 不变量与 fallback (v17)

**新增不变量(v17)**:

1. **VOLATILE↔REPLAYING_CONSOLIDATING 振荡上限**:同 Statement `replay_count >= MAX_CONSOLIDATION_ATTEMPTS`(默认 5)时,Replay Scheduler 不再选中其退回 VOLATILE,而是强制转 CONSOLIDATED 且 `review_status=PENDING_REVIEW`,emit `statement.consolidation_forced(reason="max_attempts")`。理由:防止单条高 salience 但证据不足的 Statement 永久占用 Replay 采样预算。

2. **VOLATILE TTL 兜底**:Statement 进入 VOLATILE 超 `T_max_volatile`(默认 7 天)且不在 Affect Buffer,定时清理任务自动 decay 至 ARCHIVED,emit `statement.archived(reason="volatile_ttl_exceeded")`。理由:兜底从未被 Replay 选中的低 salience 长尾。

3. **REPLAYING_CONSOLIDATING fallback timeout**:进入该状态后超 `T_replay_consolidating_timeout` 自动回退 VOLATILE,emit `statement.replay_timeout`。RuntimeHealth=READY 时默认 1 小时,DEGRADED/DRAINING 时默认 4 小时(避免重启狁蛇)。与 REPLAYING_RECONSOLIDATING 的 30 分钟可塑窗口超时对称。

4. **未结清 Commitment 保护**:`commitment.active_holding` 事件 Consumer(decay scheduler)在 in-memory set 中持有相关 stmt_id,decay 候选选取时 O(1) 排除。terminal 态时 emit `commitment.released` 从 set 移除。详见 §12.2。

5. **T5/T8 并发互斥**:CONSOLIDATED Statement 同时被 Retrieval 召回(T5)与 Replay decay(T8)触发时,decay 不再同步写库,改 emit `statement.decay_candidate` 经事件总线。outbox dispatcher 对同 stmt_id 顺序串行处理(§5.4 同 aggregate 顺序投递契约的直接应用)。后到事件读到 state 已变即幂等跳过。
...
**Container dimension key 表(v15.1 新增)**:
...
`Bus.rebuild_container(dimension=...)` 的 `dimension` 参数必须属于该 Container 子类声明的 key 表。未声明 dimension 必须拒绝写入并 emit `container.dimension_invalid` 或写入同等审计错误。P0/P1 可用整体 rebuild + 单 `version` CAS;维度级 version/sequence 是 P2+ 优化,不是 P0 必备框架。
...
### 3.8 EngramStore:不可被静默覆盖的原档(v6:合规生命周期)

> **v5 修复**:v4 把 EngramStore 既描述为"Statement.evidence 指向的独立 BlobStore"(§3.8)又描述为"Hippocampus 的内部分层"(§6.1)—— 不一致。v5 把 EngramStore 定位为**与 Hippocampus / Neocortex 平级的全局 BlobStore 子系统**,见 §6.0。
>
> **v6 修复**:v5 的"EngramStore 永不删"与用户撤回/法务 purge/retention_policy 冲突。v6 改为:EngramStore 的**审计元数据 append-only**,但 verbatim 内容是否可恢复由 `retention_mode` 决定。

任何 Statement 的 `evidence` 必须指向 EngramStore 中的原始证据片段。Engram 的 id/source/hash/audit log append-only,即使 LLM 抽取错误也能追溯;但当原文包含个人数据或敏感信息时,FORGOTTEN 不能只是"检索隐藏"。v6 要求每条 Engram 声明保留模式:
...
多 Cognizer 可共享同一 Engram(参考 Letta Archive 模型,§16.2),但共享记录的 purge 采用"最严格访问者 wins":任一主体触发 `crypto_erasure` 时,共享记录必须拆分引用或整体加密擦除,不能继续向其他 holder 暴露原文。
...
class Engram:
    ...
    segment_map: list[SourceSegment] = []    # P2+ 片段级 offset/role/speaker/observed_at;P0 可为空
...
SourceAdapter 若执行分块、转录、PDF 解析、episode 合并或 multimodal segment 提取,必须输出 `segment_map`。P0 direct_api 可不输出 `segment_map`,Extractor 以 `(engram_ref, chunk_index, Engram.timestamp/content_hash)` 构造最小 `SourceSpanRef`。凡引用 `segment_id/span_start/span_end` 的 Statement,必须能在 `segment_map` 中找到对应片段;不能凭 LLM 自造 offset/episode id。若 adapter 无法给出片段级时间,Engram.timestamp 可作为 fallback,但所有相对时间抽取默认 `review_status=INFERRED_UNREVIEWED`。
...
借鉴 mempalace RFC 002 的 source adapter contract,Starling 不再把"原文"当作所有入口的无条件承诺。每条 Engram 必须声明来源适配器、ingest mode 与转换列表:

| 字段 | 规则 |
|---|---|
| `adapter_name/adapter_version` | 写入源的稳定名称与版本;手写/API 直写也必须使用内置 `direct_api` adapter |
| `ingest_mode` | `chunked_content` 表示源内容分块;`whole_record` 表示一条源记录完整入 EngramStore;`metadata_only` 表示内容由用户/adapter 合成,不可当 verbatim 证据 |
...
| `privacy_class` | adapter 默认隐私级别,可被 source policy 升级;影响 retention、trace retention 与 Engram raw access |

`Statement.evidence` 引用 `metadata_only` EngramStore 时,Context Pack 必须标 `evidence_kind=metadata_only`,不得把它包装成一手原文。若 adapter 声明 `declared_lossy`,Extractor 的 confidence 初值上限默认 0.8,除非另有独立 byte-preserving evidence 支持。
...
- `tom_inferred` 虽会 emit `statement.written`,但 Replay 采样权重必须乘以 `tom_inferred_factor=0.25`;若按 `derived_from` 递归展开的派生链深度 ≥ 3,或 `causation_chain` 已含 ToM→Replay→Container,ToM Engine 暂停对该链路增量推断,只保留显式查询时即时 perspective_take。

**provenance 不变量(v15.1 新增)**:
- Statement 的 `provenance` 在 `Bus.write` 首次提交时确定,写入后不可变。
- 后续 confidence 调整、`review_status` 提升/降级、`access_count` 累加、`consolidation_state` 迁移、evidence erasure 标记,均不得改变原 Statement 的 provenance。
...
| BlobStore(EngramStore) | append-only,不可覆盖 | S3 / 本地 fs / Letta archival |
...
class ProfileCapability(BaseModel):
    ...
    stage_timing: bool
    cross_partition_transaction: bool       # v17:Hippocampus/Neocortex/Outbox 三分区是否原子提交
```
...
- `production` profile 不得加载 testing helper 包;helper 包必须放在 `starling.testing.*` 或等价隔离命名空间,且 CI 静态扫描禁止 production entrypoint/import graph 引用该命名空间。
...
- 启用共享 Engram 时必须满足 `drawer_refcount=true` 或提供等价的引用拆分策略。
...
- P0 默认单 tenant,但 `Cognizer.tenant_id` 与 `Statement.tenant_id` 仍必须落库,默认值为 `"default"`。所有 SQLite 查询模板必须显式包含 `tenant_id` 谓词;测试 adapter 可用 final query assertion 检查占位符存在,缺失即拒绝执行。
- 能力检查结果写入 `system.profile_checked` outbox 事件与 `RuntimeHealth.capability_status`,供 smoke test 与运维审计读取。

**cross_partition_transaction(v17 新增)**:
声明该 profile 是否支持 Hippocampus / Neocortex / Outbox 三个逻辑分区的原子事务提交。Reconsolidation severe path(§11.2)需要新版 Statement + supersedes 边 + 旧版 ARCHIVED + outbox 事件 4 项同事务。

- `local-lite` (SQLite): true(单库事务自然原子)
- `local-graph`: true(SQLite 主表 + 内置图引擎单事务)
- `cloud-graphiti` (Postgres + Neo4j + Qdrant): false(跨库不能原子)→ 必须走 saga 补偿路径(§11.2)
- `letta-bridge` / `cognee-bridge`: 取决于底层,声明时按宿主能力填

false 时 §11.2 severe path 必须实现 saga 补偿:先写新版与 supersedes,后 emit 事件,最后将旧版 ARCHIVED;任一步失败 emit `reconsolidation.compensated` 撤销已完成步骤,旧版保持 CONSOLIDATED。
...
| 投影 | 主键/分片 | 面向查询 | 更新来源 |
|---|---|---|---|
| `idx_holder_state_time` | `(holder, consolidation_state, valid_from, valid_to)` | FACT/HISTORY 基础过滤 | statement.written/derived/corrected/consolidated/superseded/archived/forgotten |
| `idx_holder_subgraph` | `(holder, consolidation_state, modality, subject)` | holder 子图快速过滤;FACT_LOOKUP/BELIEF_OF_OTHER 前置过滤 | statement.written/derived/corrected/consolidated/superseded/archived/forgotten |
...
**P0 主表必要索引**:
P0 虽不实现 Projection Index,但写入热路径必须有 `idx_statement_id_tenant(id, tenant_id)`。Validator 检查 `derived_from`、跨 tenant 派生、SUPERSEDES 旧版和 evidence 传播时只能走点查,不得退化为全表扫描;该索引建在 Statement 主表上,不属于 P2 投影索引。
...
| 投影 | 订阅事件 | 更新动作 |
|---|---|---|
| `idx_holder_state_time` | `statement.written/derived/corrected/consolidated/superseded/archived/forgotten` | 写入/派生/修正/巩固/版本替代时增量 upsert;archived 标记降权;forgotten 物理删除 |
| `idx_holder_subgraph` | `statement.written/derived/corrected/consolidated/superseded/archived/forgotten` | 同 holder 子图维度 upsert;state/version 变更同步;archived 保留但降权;forgotten 删除 |
...
1. `append_evidence(engram_input)` — 创建 Engram,做 source trust、权限、retention、审计和幂等。
...
```
append_evidence(input) → Bus → EvidenceValidator → EngramStore → outbox(evidence.appended)
...
枚举定义见 §3.8 Engram schema。

规则:
- `system_internal / observer_agent / replay_output` 默认 `NO_STORE`,只写 audit counter,不得创建普通 Engram;debug profile 可显式 allowlist 到 `STORE_METADATA_ONLY`,但不可参与 holder 画像、Replay、ToM。
- RetrievalReceipt、PipelineRun trace、Extractor raw prompt/response、worker log、health event 默认都是 `system_internal`;若用于评测,必须进入隔离 evaluation tenant 或 metadata-only EngramStore。
- `tool_observation` 只有在工具输出是用户可见事实或外部世界观测时可 `STORE`;纯执行日志、stack trace、token usage 默认 `STORE_METADATA_ONLY` 或 `NO_STORE`。
- 任何从 `replay_derived`/Container rebuild 生成的摘要不得再次作为 evidence 写入 EngramStore;其证据链应回指原始 DrawerRefs,防止摘要反喂摘要。
...
1. **Schema 校验**:`tenant_id/holder/perspective/modality/polarity/observed_at/provenance/review_status` 必填;`evidence` 与 `derived_from` 至少其一非空,并按下表校验。
...
**evidence / derived_from 分级规则(v15.1 新增)**:

| provenance | evidence 要求 | source_spans 要求 | derived_from 要求 |
|---|---|---|---|
| `user_input` | ≥1 条 EngramRef,必填 | ≥1,必填(P0 chunk 级即可) | 可为空 |
...
P0 阶段 `temporal_anchor=None` 可被 Validator 接受;序列化输出时若为空,按 `source_spans[0].observed_at` 推导。P2 启用 `segment_map`、多片段聚合或多源不一致处理后,直接抽取 Statement 的 `temporal_anchor` 必填,Validator 拒绝 None。

直接抽取 Statement 的 `tenant_id` 由 `holder.tenant_id` 派生并写入后不可变;派生 Statement 默认继承直接前驱集合中主前驱的 tenant。跨 tenant 派生规则(v16):
- 前驱集合包含 `tenant_id ∈ {"org_default","system"}` 时,允许派生 Statement 继承请求方/holder tenant,但必须在 audit metadata 标 `cross_tenant_derived=true` 与前驱 tenant 列表。
- 前驱集合涉及多个 user tenant 且存在显式跨 tenant 协议时,允许写入但 `review_status=REVIEW_REQUESTED`,由策略或人工确认最终 tenant。
- 前驱集合涉及多个 user tenant 且无显式协议时,Validator 拒绝写入并 emit `statement.review_requested` 或 `extraction.failed(schema_invalid)`。
- `tenant_id` 写入后不可变;任何 UPDATE tenant_id 的路径都必须 fail-closed。

**抽取失败补偿(v7 新增)**:
`evidence.appended` 已提交而 Extractor 产出的 Statement 被 Validator 拒收时,不得静默丢弃该 Engram。Extractor 必须发布:
...
- 超过重试阈值 emit `extraction.dead_lettered`,Engram 保留 `extraction_status=FAILED`,后续 Sleep/人工 audit 可重新抽取。

**重试去重(v8 新增)**:
LLM 抽取非确定性意味着同一 Engram 重试可能产出不同候选。Extractor 在重试前必须读取 `extraction_attempt_id` 对应的已写入结果:
- 若上次候选中部分 Statement 已通过 Validator 并写入,重试只覆盖失败片段,不得重新写入已成功片段。
- 每个候选 Statement 带 `extraction_span_key = hash(engram_ref, chunk_index, normalized_span, predicate, canonical_object)`;Bus.write 对同一 `extraction_span_key` 做幂等 upsert。
- 若重试产出与已成功 Statement 同 span 但内容不同,进入 `REVIEW_REQUESTED`,不自动 supersede。
- 一个 Engram 的最终 `extraction_status` 为 `PARTIAL_SUCCESS / SUCCESS / FAILED / DEAD_LETTERED`,用于 Sleep/人工 audit 续跑。

`normalized_span` 阶段定义(v15.1 新增):
- P0:`normalized_span = chunk_index`。同一 chunk 内多个 Statement 由 `(predicate, canonical_object)` 区分,不依赖 LLM 输出文本。
- P2 启用 `segment_map` 后:`normalized_span = (segment_id, span_start, span_end)`,用于字符级精确去重。
- P0 若 LLM 输出多个同 `(chunk_index, predicate, canonical_object)` 的候选,第一条通过 Validator 的候选接受,后续候选标 `REVIEW_REQUESTED`,不自动 supersede。

**ExtractionAttempt 明细(v11 新增,v14 收敛)**:
借鉴 mem0 的 memory history、mempalace sweeper 的确定性 id/cursor、memU workflow 的 step trace,Extractor 每次消费 `evidence.appended` 都必须记录一条 attempt,即使没有产生任何 Statement。无结果也是事实,不能让 Engram 看起来"从未处理"。

v14 明确:ExtractionAttempt 不是第二套任务生命周期,而是 `PipelineRun(kind="extraction")` 的 item-level 明细。调度、重试、lease、dead-letter、取消与最终终态以 PipelineRun 为权威;ExtractionAttempt 只解释单个 Engram/片段抽取为什么成功、失败、NOOP 或部分成功。
...
- `prompt_input_hash` 覆盖 EngramStore hash、chunk/span、extractor config、existing_ref_map 与 holder context;同 hash 的重放必须幂等。
...
| 项 | 规则 |
|---|---|
| 产出上限 | 每条 Engram 默认最多产出 10 条 Statement;超过上限按 salience / stakes / explicitness 排序截断,剩余候选进入 review queue,不自动落库 |
...
| evidence 绑定 | 直接抽取 Statement 必须绑定至少 1 个 EngramRef;派生 Statement 必须绑定 `derived_from/evidence_chain`,最终可追溯到 EngramRef |
| source span 绑定 | 直接抽取 Statement 必须绑定 ≥1 个 `SourceSpanRef`;P0 可用 `(engram_ref, chunk_index, observed_at, source_hash)`。若引用 `segment_id/span_start/span_end`,必须能在 Engram.segment_map 中找到对应片段。Graphiti 式多 episode 事实需记录所有 episode span,并以第一归因 episode的 observed_at 作为默认 temporal_anchor。派生 Statement 不得伪造直接 source span |
...
**holder_perspective(v17 新增规则)**:Extractor 必须按以下规则填值:

| 场景 | holder_perspective | 例 |
|---|---|---|
| 主语为 self(agent 自己)且 present_in_episode | FIRST_PERSON | "我认为 X" |
| 主语为他人,原文是直接引语或明确转述 | QUOTED | "Alice 说:'X'" / "Alice 说她认为 X" |
| 转述但不能追溯到原始来源(听说/传闻) | HEARSAY | "听说 Alice 认为 X" |
| LLM 推断生成的填空证据(无原文支持) | INFERRED | LLM 看上下文推断 Alice 应该认为 X |

Extractor LLM prompt 必须显式包含上述判定逻辑。perspective filter(§9.3 perspective_take)依赖该字段执行隐私边界检查,**不允许默认值兜底**(默认 INFERRED 会让 FIRST_PERSON 例被错误标记为不可靠,污染 Context Pack)。
...
| 函数 | 输入 | 规则 |
|---|---|---|
| `canonicalize_object` | Value / EntityRef / CognizerRef / StatementRef | StatementRef 一阶展开按 v17 修复规则;EntityRef/CognizerRef 取 canonical_name + id;Value 按 §3.2 Value 表标准化;list/dict 不允许兜底 hash |
...
**StatementRef 一阶展开(v17 修复)**:`canonicalize_object(StatementRef)` 计算公式为:
...
hash(
  ...
  inner.predicate,
  inner.polarity,
  inner.modality,                        # v17 新增
  canonicalize_object(inner.object),     # v17 新增:递归一层,不展开 inner.object 的 inner
)
```

修复理由:v16 一阶展开忽略 modality 与 object,导致 `A BELIEVES B likes coffee` 与 `A BELIEVES B likes tea` 哈希相同(polarity 相同),ConflictProbe 会错误合并为同一冲突候选。补 modality 区分 BELIEVES vs DOUBTS;补 inner_object 区分 coffee vs tea。

仍保留"只展开一层"规则(避免无限递归):`canonicalize_object(inner.object)` 内部对再嵌套的 StatementRef 仅返回 `inner.id_hash`,不递归。这保证二阶 ToM 的冲突检测准确,三阶以上仍受 §9.2 `derived_depth ≤ 3` 限流保护。

**ConflictProbe P0 主表索引契约(v15.1 新增)**:
ConflictProbe 是 P0 写入路径的一部分,不属于 P2 Projection Index。SQLite 主表必须提供以下同义索引,保障每次写入前的候选查询不会退化为全表扫描:
...
`canonical_object_hash` 是 Statement 主表派生列,写入时由 `canonicalize_object(stmt.object)` 计算并持久化,禁止每次 ConflictProbe 查询时运行时重算。`canonical_object_hash_version` 记录 canonicalize 规则版本,默认 `"v1"`;ConflictProbe 只对同版本 hash 做 direct comparison。若新旧 Statement hash version 不同,跨版本对象视为不可直接比较,最多降级为 `partial_overlap + REVIEW_REQUESTED`,不得触发 direct contradiction 或自动 supersede。升级 canonicalize 规则时使用新版本号(如 `"v2"`),旧行保持原版本,后台 maintenance 异步 rebuild 并写审计 watermark。

**hash version 升级期协议(v16 收口)**:
canonicalize 规则升级后,maintenance rebuild 未完成前,受影响 `(tenant_id, holder, modality, subject, predicate)` 写入进入 hash-mixed 区间。此时 ConflictProbe 必须二选一:
1. **双查协议**:新写入同时计算当前版本 hash 与上一版本 compatibility hash,分别查询两个版本候选;只有同版本命中才允许 direct_contradiction/superseding,跨版本命中只能标 `REVIEW_REQUESTED`。
2. **保守协议**:若无法计算上一版本 hash,该 key 范围内所有新写入只能落 `REVIEW_REQUESTED`,不得自动判定无冲突。

maintenance 完成并写入 watermark 后,该 key 范围恢复单版本查询。禁止在 hash-mixed 区间因新版本查无候选而认定无冲突。

**升级期超时告警(v17)**:`canonical_object_hash` 版本升级期间(maintenance rebuild 进行中),ConflictProbe 走双查协议或保守协议(§5.2 v16 已定义)。新增超期监控:

- maintenance rebuild 启动时记录 `upgrade_started_at`
- 超过 14 天未完成,emit `projection.upgrade_overdue` 事件,Consumer=Ops
- 同时 RuntimeHealth 进入 DEGRADED(不阻止 ConflictProbe 继续工作,但 Ops 可见性被强制提升)
- rebuild 完成后 emit `projection.upgrade_completed`,RuntimeHealth 回 READY

设计理由:双查热路径成本 2×,若 Ops 拖延 maintenance,系统长期 2× 负载。14 天上限是软杆,保证 Ops 不会"忘了"。

时间 overlap 使用闭开区间条件:`start <= query_end AND (end > query_start OR end IS NULL)`;`UNKNOWN_INTERVAL` 只能参与低置信 partial overlap,不得触发 direct contradiction。
...
**BusEvent envelope(v16 新增最小契约)**:
每个 outbox 事件必须包含 `event_id / event_type / primary_id / aggregate_id / outbox_sequence / causation_chain / idempotency_key / payload`。`primary_id` 是业务幂等默认键的主标识,必须由 producer 明确填写:Statement 事件取 `stmt_id`,Container 事件取 `container_id`,Evidence 事件取 `engram_ref`,Conflict 事件取 `canonical_conflict_key`,Pipeline 事件取 `run_id`。`aggregate_id` 用于同 aggregate 顺序投递,可等于 primary_id 或更粗粒度的 run/window id;Subscriber 不得假设跨 aggregate 顺序。

**aggregate_id 默认表(v16 收口)**:
...
`belief.conflict` 事件可能早于某些相关 `statement.*` 事件被对应 subscriber 看见;Reconsolidation/Container Builder 消费冲突事件时必须按 stmt_id 补查主表,以主表事务状态为准,不能只相信事件到达顺序。

| 事件 | Producer | Consumer | P 阶段 | 触发时机 |
|---|---|---|---|---|
| `evidence.appended` (v6 新增) | Bus.append_evidence | Extractor / Cognizer Hub / Audit | P0 | Engram 事务提交 |
| `evidence.redacted` (v6 新增) | Compliance Engine | Retrieval index / Audit | P2 | Engram 转 redacted_retain |
| `evidence.erased` (v6 新增) | Compliance Engine / KMS | Retrieval index / Audit | P2 | Engram crypto_erasure 完成 |
| `extraction.failed` (v7 新增) | Extractor / Validator | Extractor retry / Audit / Review queue | P0 | evidence 已提交但 Statement 抽取被拒 |
| `extraction.retry_scheduled` (v7 新增) | Extractor retry policy | Extractor | P0 | parse/schema 失败进入延迟重试 |
| `extraction.dead_lettered` (v7 新增) | Extractor retry policy | Audit / Review queue | P0 | 抽取重试耗尽或 privacy_violation |
| `statement.written` | Bus(写完 Hippocampus 后) | Replay / ToM / Prospective / Affect Buffer | P0 | 用户输入 / ToM 推断进 VOLATILE 后 |
| `statement.derived` (v5 新增) | Bus(写完 Neocortex 后) | ToM / Container Builder | P0 | Replay 派生 stmt 直接进 CONSOLIDATED 或 candidate index |
| `statement.corrected` (v6 新增) | Reconsolidation transaction | ToM / Container Builder / Retrieval index | P2 | severe correction 新版和旧版原子提交 |
| `statement.recalled` | Retrieval Planner | Reconsolidation(异步开窗判定) | P2 | 检索命中 |
| `statement.consolidated` | Replay Scheduler | ToM(更新 Persona) | P2 | REPLAYING_CONSOLIDATING → CONSOLIDATED |
| `statement.superseded` | Reconsolidation transaction | ToM / Container Builder | P2 | REPLAYING_RECONSOLIDATING 产 supersedes 链 |
| `statement.archived` (v5 新增) | Replay (decay) / Reconsolidation (contradict-severe) | Retrieval index(降权) | P2 | CONSOLIDATED→ARCHIVED 或 REPLAYING_RECONSOLIDATING→ARCHIVED |
| `statement.review_requested` (v14 新增) | Validator / Compliance / Reconsolidation | Review queue / Retrieval index / Audit | P1 | 低置信、合规擦除影响或派生链缺证据时进入人工/规则复核 |
| `statement.forgotten` (v5 新增) | Compliance Engine | Retrieval index(剔除) / Audit log | P2 | ARCHIVED→FORGOTTEN(显式 purge) |
| `statement.references_existing` (v17 新增) | Bus.write 检测 derived_from 含 CONSOLIDATED/ARCHIVED 时 | Reconsolidation Engine | P2 | 闭环 §11.1 触发路径 2,supporting/contradicting 两类 |
| `statement.decay_candidate` (v17 新增) | Replay decay | Bus dispatcher (per-stmt 串行) | P2 | 替代同步 decay 写库,消除 T5/T8 race |
| `statement.consolidation_forced` (v17 新增) | Replay Scheduler | Audit | P2 | replay_count >= MAX 强制升级 |
| `statement.replay_timeout` (v17 新增) | RuntimeHealth monitor | Audit | P2 | REPLAYING_CONSOLIDATING fallback timeout 触发 |
| `commitment.fire` | PolicyEngine(经 Bus) | Prospective(下发提醒) | P3 | Trigger 命中 |
| `commitment.fulfilled` (v5 新增) | Prospective(检测 fulfilled_by) | Reconsolidation(↑ confidence) / Cognizer Hub(↑ trust_priors) | P3 | Commitment.state ACTIVE→FULFILLED |
| `commitment.broken` (v5 新增) | PolicyEngine(deadline 过) | Reconsolidation(↓ confidence) / Cognizer Hub(↓ trust_priors) | P3 | Commitment.state ACTIVE→BROKEN |
| `commitment.renegotiated` (v5 新增) | Validator(检测 RECANT+重新 COMMIT) | Prospective / ToM | P3 | Commitment.state ANY→RENEGOTIATED |
| `commitment.withdrawn` (v6 新增) | Validator / Prospective | Prospective / Retrieval index / Container Builder | P3 | Commitment.state ACTIVE/BROKEN/RENEGOTIATED→WITHDRAWN |
| `commitment.active_holding` (v17 新增) | PolicyEngine 转 ACTIVE 时 | decay scheduler | P3 | decay 候选 O(1) 排除未结清承诺 |
| `commitment.released` (v17 新增) | PolicyEngine 转 FULFILLED/WITHDRAWN 时 | decay scheduler | P3 | 释放 active_holding 保护 |
| `container.rebuilt` (v6 新增) | Bus.rebuild_container | Working Set / Retrieval cache | P2 | Persona/CommonGround 物化视图 CAS 成功 |
| `container.dimension_invalid` (v15.1 新增) | Bus.rebuild_container | Audit / Ops | P2 | Container rebuild 请求使用未声明 dimension key |
| `action.policy_blocked` (v14 新增) | ActionPolicyGraph / PolicyEngine | Audit / Ops / Prospective | P5 | 外部动作违反 required-before-exit、requires-approval、max-count 或 allowlist |
| `cognizer.observed` | Bus(写入消息时) | ToM(更新可见性) | P1 | 主体上线/发声 |
| `belief.conflict` | ConflictProbe | Reconsolidation(优先重放仲裁) | P2 | 探针发现矛盾 |
| `norm.violated` | NormChecker | Affect Buffer(高 salience) / Audit | P3 | 规范违反 |
| `extraction.noop` (v11 新增) | Extractor | Audit / PipelineRun | P0 | Engram 已处理但无新 Statement |
| `runtime.health_changed` (v11 新增) | RuntimeHealth monitor | Ops / Scheduler gates | P0 | READY/DEGRADED/DRAINING/UNREADY 变更 |
| `pipeline.run_started/completed/failed/cancelled` (v11 新增) | PipelineRun ledger | Ops / Replay / Projection / Audit | P0 | 长任务生命周期;completed payload 必须携带 `COMPLETED/PARTIAL_SUCCESS/DEGRADED_COMPLETED` |
| `projection.rebuild_started/completed/failed` (v11 新增) | Projection worker | Retrieval / RuntimeHealth | P2 | 投影索引重建或水位修复 |
| `projection.upgrade_overdue` (v17 新增) | maintenance rebuild monitor | Ops | P0 | canonical_object_hash 升级 >14 天 |
| `system.delivery_failed` (v6 新增) | Outbox dispatcher | Ops / Audit | P0 | Subscriber 重试超过阈值 |
| `system.runaway` (v17 新增) | Bus | Ops + Audit | P0 | causation_chain depth >3 时触发,payload=[chain_root, depth, source_event_id] |
...
6. **业务幂等窗口**:Subscriber 除投递幂等外,还必须对用户可见副作用声明业务幂等键与窗口。P0 默认只冻结 `commitment.fire` 与 `statement.recalled`;其他事件使用一次性业务键。
...
**causation_chain 传播规则(v15.1 新增)**:
- Subscriber 处理事件 `E` 时,必须把 `E.event_id` 作为当前 `causation_parent` 上下文携带。
- Subscriber 在处理过程中调用 `Bus.write / Bus.emit / Bus.append_evidence / Bus.rebuild_container` 时,必须显式传入 `causation_parent`;根因事件显式传 `None`,不允许靠默认省略。
- Bus 生成新事件 `N` 时,若有 parent,`N.causation_chain = parent.causation_chain + [parent.event_id]`;若 parent 为 None,链为空。
- 链长检查在 Bus 发布事件时执行:`len(causation_chain) > 3` 直接拒绝派生事件,emit `system.runaway`,并在相关 `PipelineRun.warnings` 中记录。

**业务幂等默认值(v15.1 收敛)**:
不维护全局大表。P0 只冻结两个会产生用户可见重复副作用的默认键:`commitment.fire = (commitment_id, trigger_id, fire_bucket_24h)`,`statement.recalled = (query_id, query_window_2s)`。

其他事件默认使用一次性业务键 `(event_type, aggregate_id, version_or_sequence)`。v16 将其收敛为可推导规则:
- `aggregate_id = event.primary_id`;若 producer 需要更粗粒度排序,可另填 envelope 的 `aggregate_id`,但业务键仍默认取 `primary_id`。
- `version_or_sequence`:状态转换事件取 `payload.target_state` 或 `payload.dimension_version`;非状态转换事件取 `outbox_sequence`。
- Subscriber 不声明覆盖时严格使用此默认规则;覆盖必须在 Bus 配置中显式注册,未注册覆盖不生效。

**实现位置**:Bus 总线层统一拦截,Subscriber 只负责传递 `causation_parent` 与声明必要业务幂等键,不自行实现第二套 outbox。

#### §5.4.x 触发路径完整性 + 默认值冻结 (v17)

**Reconsolidation 触发路径防抖完整性(v17 显式列出)**:§11.1 列出的 5 种 Reconsolidation 触发路径**全部**适用本节防抖契约,具体到事件:

| 触发路径 | 对应事件 | 防抖键 |
|---|---|---|
| 1. statement.recalled | statement.recalled | (stmt_id, query_window_2s) |
| 2. derived_from 引用 | statement.references_existing(v17) | (target_stmt_id, source_stmt_id, window_10s) |
| 3. belief.conflict | belief.conflict | (canonical_conflict_key, window_10s) |
| 4. 显式 reconsolidate API | reconsolidate.requested | (stmt_id, request_id) |
| 5. commitment.fulfilled/broken | commitment.fulfilled / commitment.broken | (commitment_id, terminal_state) |

同 stmt_id 在 Reconsolidation 窗口期内只允许一个活跃窗口,所有触发事件追加到同一 `pending_evidence` 队列,窗口 close 时统一仲裁。该规则跨事件类型生效,不限于 belief.conflict。

**默认值冻结(v17)**:
- Replay Online 模式:`N = 3`(每 3 条 statement.written 触发一次 Online 采样)
- Replay Idle 模式:`T = 120s`(agent 空转 120 秒触发 Idle 反思)
- inbox idempotency_key 保留期:`7 天`(超期清理,Subscriber 启动时从 outbox sequence checkpoint 恢复)
- window_bucket 默认粒度:`24h`(commitment.fire 已冻结)/`10s`(belief.conflict / statement.references_existing)/`2s`(statement.recalled)
- causation_chain 深度上限:`3`(超过即拒绝派生事件并 emit system.runaway)
- MAX_CONSOLIDATION_ATTEMPTS:`5`
- T_max_volatile:`7 天`
- T_replay_consolidating_timeout:`READY 1h / DEGRADED 4h`
- canonical_object_hash 升级 maintenance:`>14 天 emit projection.upgrade_overdue`
- Commitment BROKEN 计数上限:`3`(达到自动 WITHDRAWN)
- RENEGOTIATED supersedes 链长:`≤3`(超过拒绝)
...
class Container(BaseEntity):
    ...
    version: int
    dimension_versions: dict[str, int] = {}  # P2+;P0/P1 可为空,只用整体 version
    dimension_sequences: dict[str, int] = {} # P2+;每个维度已消费的最高 outbox sequence
    last_rebuilt_at: datetime
    build_policy: Literal["on_event","sleep","manual"]
```

更新规则:
- 只有 `Bus.rebuild_container(container_id, dimension, expected_version, source_refs)` 能写 Container。
- `dimension` 必须属于 §3.6 声明的合法 key。P0/P1 使用整体 `version` CAS;P2+ 若启用 `dimension_versions`,则 `expected_version` 表示对应 dimension version。
- `container.rebuilt` 事件只表示物化视图刷新,不表示新增事实。
- Retrieval 可读 Container,但若整体 version 或 P2+ 对应 `dimension_sequences[dimension]` 落后于相关 Statement outbox sequence,必须只对该 Container/dimension 降级直接查 Statement 图。
- CAS 冲突采用指数退避 + 合并重建:P0/P1 整体重建冲突 3 次后进入 Sleep 批处理;P2+ 可只重试冲突 dimension。
...
class PipelineRun(BaseEntity):
    ...
    aggregate_id: str                       # engram_ref / holder / projection / container / tenant
    ...
    idempotency_key: str
    pipeline_name: str
    pipeline_version: str                   # P0 简单版本号;P2+ 可升级为 revision token
    step_contracts: list[dict] = []         # P2+;P0/P1 不启用通用 step graph
...
**Pipeline step contract(v12 新增,v15 延后启用)**:
memU 的 WorkflowStep / PipelineRevision 说明,长任务账本必须能解释"这次到底跑了哪个步骤版本"。Starling 的 PipelineRun 因此增加:
- P0/P1 简单实现只记录 `pipeline_name/pipeline_version/status/counters/watermark/error_kind`,不实现通用 step graph。
- `pipeline_revision`:P2+ 注册/变更 pipeline 时生成 revision token;同一 input_hash 在不同 revision 下不得复用旧结果。
- `step_contracts`:P2+ 每个 step 声明 `requires/produces/capabilities/config_hash`;run start 前验证上游 produce 覆盖下游 require,profile capability 覆盖 step capabilities。
...
full/debug trace 不得作为普通 EngramStore evidence;若因合规擦除触发,trace 中对应 raw payload 必须同步 redaction 或 crypto erasure。

---

## 6. EngramStore / Hippocampus / Neocortex:存储分层

> **v6 重写**:EngramStore 仍与 Hippocampus / Neocortex 平级,但其写入必须经 `Bus.append_evidence`,其内容生命周期必须遵守 `EngramRetentionMode`。

### 6.0 EngramStore:全局证据子系统(v6 合规版)

EngramStore 是 Starling 的最底层证据保底,**与 Hippocampus / Neocortex 平级**,不属于任何记忆子系统:

```
┌─────────────────────────────────────────────────────────┐
│  EngramStore  ── 全局 BlobStore + Audit Metadata             │
│   • 任何 Statement.evidence 必须指向其中片段            │
│   • 写入经 Bus.append_evidence,不允许直接 append        │
│   • 元数据 append-only;内容按 retention_mode 保留/擦除 │
│   • 多 Cognizer 可共享同一 Engram(参考 Letta     │
│     Archive,§16.2)                                     │
└─────────────────────────────────────────────────────────┘
        ▲                 ▲                  ▲
        │                 │                  │
...
**写入契约**:
```python
engram_ref = Bus.append_evidence(
    source=source,
    content=verbatim,
...
Extractor 只能消费 `evidence.appended` 事件产生 Statement;不得绕过 Bus 直接从文件系统读写 Engram。

**EngramStore 写入前置校验(v12 新增)**:
- `source_kind` 与 `ingest_policy` 先于 retention policy 计算;`NO_STORE` 不创建 Engram,只写 audit event。
- `metadata_only` EngramStore 不满足"verbatim evidence"要求,只能作为辅助来源;高影响 Statement 需至少一条 `byte_preserving` 或外部可信 evidence 才能自动 `APPROVED`。
...
> 设计原则:**写得快、稀疏索引、显著性优先、模式分离**。原档保底由 §6.0 EngramStore 负责,本子系统**不直接持有 verbatim 内容**,只持有引用与索引。

**内部分区**(v5 移除 EngramStore 行,EngramStore 在 §6.0):

```
┌─ Episodes ───── 按 EM-LLM 惊奇度切分的事件单元(指向 EngramStore)
...
```
discover → 在 EngramStore 中识别新 Cognizer(NER + 对话角色 + alias 归一)
...
**ToM 派生链限流(v7 新增,v15.1 收敛字段)**:
- Belief Tracker 对 `tom_inferred` 产生的持久 Statement 必须记录 `derived_from` 直接前驱,事件侧记录 `causation_chain`。
- 若 `stmt.derived_depth >= 3` 或链路中已出现 `ToM → Replay → Container → ToM`,暂停自动增量推断,只在用户显式 META_BELIEF / perspective_take 查询时即时计算。
...
def perspective_take(target: CognizerRef, query: str, time: datetime) -> Context:
    visible = filter_by_frontier(engram_store, target, time)        # 1. KnowledgeFrontier 遮蔽
...
Replay eligible set 只包含 `statement.written` 进入的 `user_input` 与可落库 `tom_inferred` Statement;`replay_derived/reconsolidation_derived` 不进入采样候选,而不是以权重 0 参与抽样。
...
- 派生深度使用 `stmt.derived_depth` O(1) 判断;若 `derived_depth >= 3`,Replay 不再把该 Statement 作为自动派生输入,只允许显式 audit/review 路径处理。
...
|---|---|---|---|---|---|
| `compress` | 多条相似 EpisodicEvent | 1 条更高层语义 Statement(EngramStore 指针保留) | `replay_derived / APPROVED` | 输入:VOLATILE→CONSOLIDATED;输出:CONSOLIDATED | ❌(v5 默认 non-propagate;借鉴 memU v1.5.0 #386,显式 propagate=True 时才级联) |
...
| `purge_compliance` | 用户撤回 / 法务事件 | state→FORGOTTEN + 传播到 derived 链 + EngramStore 按 retention_mode redacted/erased | — | ARCHIVED 或 CONSOLIDATED → FORGOTTEN(emit `statement.forgotten` + evidence 事件) | — |
...
巩固结果用 `derived_from` 指回直接输入 Statement;完整派生链由递归展开计算。原始证据按 EngramStore retention_mode 保留、脱敏或加密擦除,不再用"永不删除"作为统一规则。
...
**FORGOTTEN 的语义(v6)**:Statement 从所有普通检索索引中剔除;Engram 根据 retention_mode 执行:
...
**mild correction provenance 语义(v17 明确)**:轻微反对路径**修改原 Statement**,不产生新 Statement,不进入 supersedes 链。这与 severe path 的"原子提交新版+旧版 ARCHIVED"形成明确二分:

- 原 Statement 的 `confidence` 字段被 Bayesian update,新值落库
- `provenance` **保持原值**(例:user_input 原 Statement 经多轮 mild correction 后 provenance 仍是 user_input,不变为 reconsolidation_derived)
- `confidence_history` 字段(JSON 数组,v17 新增)追加一条 `[(old_value, ts, evidence_summary_hash)]`
- 不 emit `statement.corrected`,只 emit `statement.consolidated`(语义为"窗口 close,版本回归 CONSOLIDATED")

设计理由:轻微 confidence 调整高频发生(每条 user-stated belief 可能被多轮 supporting/contradicting evidence 调整),若每次都产生新版本会让 supersedes 链被被动拉长 O(N),也违反 §3.10 provenance 不变量。`confidence_history` 提供审计轨迹替代版本号。

仅当 contradict-severe 时才走"产新版+旧版 ARCHIVED"路径,产生 reconsolidation_derived provenance。

**saga 补偿路径(v17,cross_partition_transaction=false 时启用)**:

```
步骤 1:写入新版 Statement(reconsolidation_derived,CONSOLIDATED)
步骤 2:写入 SUPERSEDES 边(新版 → 旧版)
步骤 3:旧版 state 改 ARCHIVED
步骤 4:emit statement.corrected + statement.archived + statement.superseded(同 outbox batch)

任一步失败:
  - 步骤 1 失败:无副作用,直接报错
  - 步骤 2 失败:删除步骤 1 的新版,emit reconsolidation.compensated
  - 步骤 3 失败:删除步骤 1 新版 + 步骤 2 边,emit reconsolidation.compensated
  - 步骤 4 失败:已写入数据保留,但需 audit log + Ops 告警(数据一致但事件未发,需手动重发)
```

旧版状态在补偿成功后保持 CONSOLIDATED,系统语义不变。
...
1. **BROKEN 计数自动 WITHDRAWN**:同 commitment_id 进入 BROKEN 状态累计 `>= MAX_BROKEN_COUNT`(默认 3)次后,下一次 deadline 过期不再转 BROKEN,而是自动 WITHDRAWN,emit `commitment.auto_withdrawn(reason="chronic_failure")`。同时 Cognizer.trust_priors[holder] 下调(具体公式见 §8.3)。设计理由:防止单一 commitment_id 长期占用 supersedes 链与 Replay 资源;反映"反复失信后系统不再跟踪"的人类心智语义。

2. **RENEGOTIATED 链长上限**:Commitment supersedes 链长 `>= 3` 时,新的 RENEGOTIATED 请求被 Validator 拒绝,emit `commitment.renegotiation_blocked`,要求先 WITHDRAWN 再重新 COMMIT。设计理由:supersedes 链 O(N) 展开成本(§9.2 ToM 推断、§10.4 衰减公式依赖)需可控;3 次 RENEGOTIATED 已是协商上限的合理阈值。

**active_holding 反向保护事件(v17 新增)**:

Commitment 转 ACTIVE 状态时,PolicyEngine emit `commitment.active_holding(commitment_id, related_stmt_ids)`。Consumer 是 Replay decay scheduler,后者在 in-memory set 中持有 stmt_id 集合。decay 选取候选时 O(1) 排除该 set 内 Statement(§3.4 例外条款"未结清 Commitment 不允许相关 Statement 进入 ARCHIVED"的 enforcement 入口)。

Commitment 转 FULFILLED/WITHDRAWN 终态时,emit `commitment.released(commitment_id)`,decay scheduler 从 set 移除。

dispatcher 重启时必须 boot replay 最近 7 天的 commitment.active_holding / commitment.released 事件,重建 in-memory set。否则未结清 commitment 关联 Statement 可能被误 ARCHIVED(critical failure mode,见 §11.x 测试 TC-A9-003)。
...
### 12.5 ActionGuard:前瞻动作护栏(v13 新增,v15 简化)

Prospective Loop 可以只提醒用户,也可能在产品化后触发工具或外部动作(发通知、开 issue、创建日历、调用业务 API)。提醒可轻量;真实动作先用最小 `ActionGuard`。完整 `ActionPolicyGraph` 只有在需要多步前后置、条件分支或复杂工具链时(P5+)才启用。
...
规则:
- `commitment.fire` 默认只把 pending reminder 注入 Working Set。
- 若要调用外部 tool/action,必须满足 `action_name in allowed_actions`。
- 命中 `requires_approval` 的动作进入 human approval 队列,不得由 LLM 自行确认。
- 每个外部动作必须有 `business_idempotency_key`,默认窗口沿用 §5.4。
- Guard 失败默认 fail-closed,emit `action.policy_blocked`,并在 `PipelineRun.warnings/counters` 中可见。

完整 ActionPolicyGraph(P5+ 可选):
...
规则:
- `commitment.fire` 只能把 pending reminder 注入 Working Set;若要调用多步外部 tool/action,必须经 `ActionPolicyGraph.allowed_next_actions(history, available_actions)`。
...
完整 `RetrievalScopePlan` 属于 P4+。P0/P1 只实现固定 `basic_retrieve` 路径,不需要通用 planner。
...
- v15 简化规则:P0/P1 的代码不需要创建 `RetrievalScopePlan` 对象;Receipt 可用固定字段记录 `scope="statement_main"`、filters、candidate_counts 与 sufficiency。
...
4. fetch:    并发执行多源(向量 / 图 / KG / EngramStore / Working Set / ToM API)
...
- merge 阶段只处理 StatementRef/score/metadata,不得重新打开未授权 Engram raw 内容。
...
- Progressive plan 先查 `working_set / statement_main / projection_index` 等低成本 scope;若 `sufficiency_status=SUFFICIENT` 且 stop_policy 允许,跳过 `graph_index / semantic_index / engram_evidence`。
- Scope 被跳过必须写 `skipped_scopes` 与 `stop_reason`;不能让 Receipt 看起来像全量检索。
- Scope 失败只能按该 step 的 `on_error` 降级;不得自动扩大 holder/group 范围。
- `NEEDS_RAW` 不是直接打开 EngramStore,而是触发 raw evidence gate;gate 失败则 `ABSTAINED` 或 `MISSING_INFO`。

### 13.4 Intent → Path 映射

| Intent | 主路径 | 辅助 |
|---|---|---|
| FACT_LOOKUP | Neocortex Semantic(holder=self) + EngramStore 证据 | Working Set |
| BELIEF_OF_OTHER | Neocortex Semantic(holder=target) | EngramStore 中 target 发言 |
...
P0 验证目标是"Bus.append_evidence → Extractor → Bus.write → direct state transition helper → basic_retrieve"可跑通,避免只冻结 schema 而没有端到端读路径。P0 不实现 Replay Scheduler;验收中需要 CONSOLIDATED 状态时,使用 testing profile 的直接状态迁移 helper 模拟巩固完成,并记录审计事件。
...
- `scope_plan` 必须能解释每个 scope 的 filter 来源、holder/group、fallback 与 stop policy;若 plan 因 `working_set/statement_main` 已足够而短路,`skipped_scopes` 必须列出未查的 `semantic_index/graph_index/engram_evidence`。
- `score_breakdown` 只记录可审计分数与 StatementRef,不泄露被遮蔽证据正文。
- `sufficiency_status=SUFFICIENT` 表示 Context Pack 足以回答;`MISSING_INFO` 表示已检索但缺关键证据;`NEEDS_RAW` 表示摘要/Statement 不足,需要打开 Engram raw evidence,此时必须先通过 retention/visibility 检查并在 receipt 记录 raw-evidence request;`ABSTAINED` 表示主动拒答。
...
Query(intent=FACT_LOOKUP, perspective=user, text=...) → Planner
   ...
   ↓ fetch:
       ...
       EngramStore time-window: 关联 Alice 4/15 原话 + 历史 Bob 持有 auth 的证据
...
[FACT]    Bob 已不再负责 auth,现由 Carol 接手(Alice 4/15 群聊宣布,evidence: EngramStore:abc123)
...
- **可解释性**:当 agent 输出错误信念时,用户能否追溯到具体 Statement + evidence(EngramStore 指针)。度量:错误定位时间。
...
| 阶段 | 时间 | 目标 | 核心交付 | 验证方式 |
|---|---|---|---|---|
| **P0 Spike** | 6 周 (v17 修订) | 可执行契约冻结 + 最小闭环 | Statement + Cognizer + 六态 ConsolidationState + StatementProvenance + ReviewStatus + EngramRetentionMode 的 Pydantic 实现与 JSON-Schema 导出;SQLite 单底座 CRUD;outbox 表;`ProfileCapability` preflight(含 `cross_partition_transaction`);最小 `PipelineRun` 账本 + `ExtractionAttempt` item 明细;`SourceKind/IngestPolicy` 与 source adapter metadata;P0 chunk 级 `SourceSpanRef`(`engram_ref/chunk_index/observed_at/source_hash`)与序列化推导的 `TemporalAnchor`;一阶 Statement 抽取 prompt(XML strict + holder_perspective 规则);`basic_retrieve(holder=self, intent=FACT_LOOKUP)` 主表直查 + 最小 `RetrievalReceipt` | 单元测试 + 50 条手工标注对话样本回归 + outbox 投递/幂等测试 + §14.1 最小变体 smoke test: Alice 宣布 Bob 不再负责 auth,testing helper 将关键 Statement 标为 CONSOLIDATED 后,检索返回 Carol 接手,并能查看 extraction attempt、pipeline run 与 retrieval receipt;19 条 v17 新增验收用例(12 CRITICAL) |
...
**P0 时间表(v17 修订)**:6 周 baseline(单/双人熟练 Python 估算),v16 原 2 周低估了以下工作量:
...
**并行性**:M0.4 与 M0.5 共享 canonicalize_object,需先完成 M0.5 的核心函数再启动 M0.4。M0.4 的 existing_ref_map 需要 M0.6 的 holder-aware 检索给 LLM 列已有候选,M0.4 第一版可用 stub 替代,M0.6 完成后回填。

**新增 M0.0**:ProfileCapability skeleton + final query assertion + testing helper marker + CI 静态扫描,作为其他所有里程碑的运行时骨架,~3 天。

总和:M0.0 (3) + M0.1 (5) + M0.2 (10) + M0.3 (3) + M0.4 (7) + M0.5 (7) + M0.6 (3) + M0.7 (14) = 52 天 ≈ 7 周。考虑 Lane B/C 部分并行(M0.4 + M0.5),收口至 6 周 baseline。

**P0 负面验收用例(v10 新增,v16 补充)**:
...
- 时间锚负例:导入 2023-06-01 的历史 Engram,文本含 "last week" → Statement.valid_from 落在 2023-05 周期,不得使用当前系统日期;若无 segment observed_at,必须低置信/待审。
- 归因不可变:写入后尝试原地修改 holder/source_speaker/perceived_by → Validator 拒绝;正确路径是 `statement.corrected + supersedes`。
- `tenant_id` 不可变:尝试原地 UPDATE Statement.tenant_id → Validator/Repository 拒绝,只能新建跨 tenant review 候选。
- `provenance` 不可变:review 提升、confidence 调整或 testing consolidate 不得修改原 Statement.provenance;fork/supersedes 新版本才可有新 provenance。
...
- P0 `temporal_anchor=None`:直接抽取可通过 Validator,序列化时由 `SourceSpanRef.observed_at` 推导;P2 profile 开启多片段后同输入必须拒绝 None。

**v17 新增验收用例(19 条,12 CRITICAL,详见 `07_v16_eng_review_test_plan.md`)**:

1. 状态机契约 5 CRITICAL:TC-A1-001/002 振荡 + TTL,TC-A5-001 fallback timeout,TC-A6-001/002 T5/T8 并发互斥
2. Commitment 契约 3 CRITICAL:TC-A2-001/002 BROKEN 计数 + 链长,TC-A9-001/002 active_holding 保护
3. Reconsolidation 契约 3 CRITICAL + 1:TC-A3-001 derived_from 防抖,TC-Q1-001 references_existing 闭环,TC-A8-001/002 severe path 双路径,TC-Q3a-001 mild provenance 不变
4. Extractor 契约 4 CRITICAL:TC-Q2-001/002/003/004 holder_perspective 四种 perspective
5. ConflictProbe 契约 1:TC-Q3b-001 二阶 Statement object 区分
6. 默认值与事件表 3:TC-A4-001 system.runaway,TC-A7-001/002 默认值 baseline

12 CRITICAL 用例为 P0 出货门槛,缺一不可。

**新增 critical failure mode 测试(v17 补,outside voice 提出)**:
- TC-A5-002:fallback 任务自身失败(双层兜底)
- TC-A9-003:dispatcher 启动 boot replay commitment.active_holding/released 事件,重建 in-memory set

P2 准入测试另列 Projection repair safety:启用 Projection rebuild 前必须构造 rebuild 抽取条数低于主表 ground truth 的用例,验证 `projection.rebuild_failed(truncation_suspected)` 且 active projection 不被替换。该用例不属于 P0 交付范围。

**P0 测试支撑 helper(v15.1 新增)**:
P0 不交付 Replay Scheduler、Compliance Engine 或 Reconsolidation Engine,但 smoke test 需要覆盖已巩固读路径与擦除标记。testing profile 可提供两个只改测试数据的 helper:
- `testing.mark_consolidated(stmt_id)`:把指定 Statement 从 VOLATILE 标为 CONSOLIDATED,用于模拟 Replay 已完成;必须写审计记录,production profile 禁用。
- `testing.mark_evidence_erased(engram_ref)`:把测试 Engram 标为 `crypto_erasure` 并同步 evidence erased 标记,用于 basic_retrieve 擦除负例;不实现完整合规传播。

这些 helper 不是生产 API,不得出现在 production profile 的路由/SDK 中。production 启动 preflight 若检测到 testing helper 注册,必须 fail-closed。

testing helper 识别硬约定(v16):所有 helper 模块文件级必须声明 `__starling_testing_only__ = True`,并位于 `starling.testing.*` 或等价隔离命名空间。production profile 启动时扫描已加载模块与已注册 helper/route 表,发现该标记即 fail-closed 并打印模块路径;CI 还必须静态扫描 production entrypoint/import graph,禁止引用 testing 命名空间。module marker 是运行时防线,不是唯一保证;不能通过 runtime flag 覆盖。
...
- 完整 RetrievalScopePlan runtime(P4;P0 receipt 只记录固定 `statement_main` 字段,不构建 plan 对象)
- ToM 推断(P1 引入一阶 schema,P4 引入二阶运行时)
- Replay Scheduler(P2;P0 只用 testing helper 模拟巩固完成,不做周期采样或 compress)
...
- 细粒度 `segment_map/span_start/span_end/segment_id`(P2;P0 只做 chunk 级 source span)
- `PipelineStepContract/requires/produces/failure_policy` 通用 step graph(P2+;P0 只记录 pipeline_name/version/status/counters)
- dimension-level Container CAS(P2+;P0/P1 可用整体 rebuild + 单 version CAS)
...
- 完整 ActionPolicyGraph(P5 可选;P3 如需外部动作只实现最小 ActionGuard)
...
| 子里程碑 | 内容 | 依赖 |
|---|---|---|
| M0.1 Schema | Pydantic 实现 + JSON-Schema 导出,含 `tenant_id`、`derived_depth`、`canonical_object_hash_version`、chunk 级 `SourceSpanRef` 与 P0 推导式 `TemporalAnchor` 序列化 | 无 |
...
- Archive(可跨 agent 共享的 ArchivalPassage 池) → EngramStore 的多 agent 共享原档池参照
...
**从 memU 迁入/复用**:
- Rust core + blob 层可作为 EngramStore 物理底座候选,但生产 profile 需先补 per-record encryption key、key shredding、shared Engram refcount
...
- sufficiency check 的 ENOUGH/MORE → Starling `RetrievalReceipt.sufficiency_status`,其中 `NEEDS_RAW` 受 EngramStore retention/visibility gate 约束
...
- Graphiti 是 episode-first:Episode 是一等输入,edge/triplet 是 episode 的产物;Starling 是 statement-first:EpisodicEvent 是 Statement 子类。迁移时不能简单一对一映射,应保留 Graphiti Episode 为 EngramStore/EpisodicEvent evidence,再把 edge/triplet 映射为 holder-aware Statement。
...
| M3 | **Major** | EngramStore 提升为独立子系统(§6.0 新增),从 Hippocampus 内部分层中移出,生命周期独立于 Statement(FORGOTTEN 后 EngramStore 仍在) | §6.0 新增、§6.1、§3.8 |
...
| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| B1 | **Blocker** | EngramStore 合规生命周期重写:`legal_hold / audit_retain / redacted_retain / crypto_erasure`;FORGOTTEN 不再默认保留 verbatim,支持 redaction 和 key shredding | §3.8、§6.0、§10.4 |
...
| M2 | **Major** | Statement schema 补 `review_status/provenance` 与派生链表达(当时为 `derivation_chain`,v15.1 已收敛为 `derived_from` 直接前驱);新增 ReviewStatus 枚举,表达派生候选和低置信审核 | §3.2、§3.10 |
...
| m2 | Minor | EngramStore 写入从 `EngramStore.append` 改为 `Bus.append_evidence`,纳入权限、source trust、retention、审计和 outbox | §5.0、§6.0、§14 |
...
| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| B1 | **Blocker** | 抽取失败补偿:新增 `extraction.failed/retry_scheduled/dead_lettered`,Engram 不因 Validator 拒收而静默失去抽取机会 | §5.1、§5.3、§14 |
...
| m1 | Minor | memU EngramStore 底座需补 per-record key/refcount 后才能支持 crypto_erasure 与共享记录 | §16.2、§20 |
...
| M2 | Major | 抽取重试去重语义:同一 Engram 重试前读取已成功片段,只重试失败片段;以 `extraction_span_key` 幂等 upsert | §5.1 |
...
| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| M1 | Major | SourceAdapter + declared transformations:Engram 增 adapter/version/ingest_mode/declared_transformations/privacy_class/byte_preserving,metadata_only 与 declared-lossy evidence 不再冒充 verbatim | §3.8、§6.0 |
...
| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| M1 | Major | SourceSpanRef + TemporalAnchor:Statement 绑定 EngramStore 片段/episode/segment,相对时间按 source observed_at/reference_time 解析,避免历史导入按系统当前日期误落地 | §3.2、§3.8、§5.1 |
...
| M4 | Major | RetrievalScopePlan scope 去外部化:核心 scope 收敛为 working_set/statement_main/projection_index/semantic_index/graph_index/container_view/engram_evidence/tom_runtime,外部 scope 只入 adapter_scope | §13.1、§13.3 |
...
> 本节基于"简单原则"复审。v14 已解决拼凑风险,但仍有若干正确却过早的泛化:完整动作策略图、通用 pipeline step graph、完整检索 scope planner、细粒度 source segment 都不应压进 P0/P1。v15 将这些机制改为"简单默认 + 明确触发后升级"。

| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| M1 | Major | 新增简单性优先原则:先固定路径/主表/单状态机,后按触发条件升级投影、scope plan、step graph、ActionPolicyGraph | §0.2、§16.1 |
| M2 | Major | ActionPolicyGraph 降级:P3 默认只实现 ActionGuard(allowlist/requires_approval/idempotency),完整图式编排延后到 P5 可选 | §12.5、§16.1 |
| M3 | Major | PipelineStepContract 延后:P0/P1 PipelineRun 只记录 pipeline_name/version/status/counters/watermark,step graph/failure_policy P2+ 启用 | §5.6、§16.1 |
| M4 | Major | RetrievalScopePlan 延后:P0/P1 不构建 plan 对象,只固定 `statement_main` 路径并写最小 RetrievalReceipt | §13.1、§13.4、§16.1 |
| m1 | Minor | SourceSpan P0 最小化:只要求 engram_ref/chunk_index/observed_at/source_hash,offset/segment_map/segment_id 延后到 P2+ | §3.2、§3.8、§5.1 |
| m2 | Minor | P0 非交付项补完整 ActionPolicyGraph、PipelineStepContract、细粒度 segment_map 与完整 RetrievalScopePlan runtime | §16.1 |

### 17.15 v15 → v15.1 修复

> 本节基于 P0 编码前契约审查。v15.1 不继续纸面扩展认知能力,只修 v15 中会让 Pydantic schema、Validator、Bus、ConflictProbe 和 P0 验收现场卡住的矛盾;同时压住过度设计,不引入生产 admin API、全局业务幂等表或第二套派生链字段。

| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| B1 | Blocker | `evidence` 硬约束按 provenance 分级:直接抽取必须有 EngramStore evidence/source span,派生 Statement 可无直接 evidence 但必须有 `derived_from`;总规则为 `evidence` 与 `derived_from` 至少其一非空且组合合法 | §3.2、§5.1 |
| B2 | Blocker | 派生链字段收敛:删除 `derivation_chain` 持久字段,只保留 `derived_from` 直接前驱;evidence_chain/derivation_chain 作为递归闭包或缓存 | §3.2、§3.10、§9.2、§10.3、§11.1 |
| B3 | Blocker | `provenance` 写入即冻结:review/confidence/state/access 后续变化不改 provenance,只有 fork + supersedes 新 Statement 才产生新 provenance | §3.10 |
| B4 | Blocker | `tenant_id` 显式进入 Cognizer/Statement schema,默认 `default`;P0 SQLite 查询模板必须包含 tenant predicate | §3.1、§3.2、§4、§16.1 |
| B5 | Blocker | P0 TemporalAnchor 简化:不强制持久化独立字段,由 chunk 级 `SourceSpanRef.observed_at` 推导;P2 多片段后再持久化 | §3.2、§5.1、§16.1 |
| M1 | Major | Container 合法 dimension key 表冻结;P0/P1 仍用整体 rebuild + 单 version CAS,维度级 CAS 保持 P2+ 优化 | §3.6、§5.5、§16.1 |
| M2 | Major | ConflictProbe P0 主表索引契约:新增 `canonical_object_hash` 派生列、`idx_conflict_lookup` 与 `idx_temporal_overlap`;明确这些不是 Projection Index | §5.2 |
| M3 | Major | 抽取幂等键 P0 定义:`normalized_span=chunk_index`;segment-level span 延后到 P2,避免依赖 LLM 输出文本 | §5.1 |
| M4 | Major | `causation_chain` 传播规则明确:Subscriber 调 Bus 必须显式传 `causation_parent`;Bus 拼链并执行链长 ≤3 检查 | §5.4 |
| M5 | Major | P0 测试支撑收敛为 testing helper:`mark_consolidated/mark_evidence_erased`,production profile 禁用;不引入生产 admin API 或完整 Replay/Compliance | §13.4、§16.1 |
| m1 | Minor | 业务幂等默认值收敛:P0 只冻结 `commitment.fire` 与 `statement.recalled` 两类用户可见副作用;其他事件默认一次性业务键,不维护全局大表 | §5.4、附录 D |

### 17.16 v15.1 → v16 修复(本版本)

> 本节基于编码前最后一次契约审查。v16 只修 M0.1/M0.2 会现场决策的问题,不引入新的认知模块或外部库概念;`derived_depth` 是性能缓存,不是恢复 `derivation_chain` 第二套字段。

| # | 严重度 | 修复内容 | 影响位置 |
|---|---|---|---|
| B1 | Blocker | Value 类型枚举与标准化规则冻结:只允许 bool/int/float/str/datetime;list/dict 拒收,避免 `canonical_object_hash` 不稳定 | §3.2、§5.2 |
| B2 | Blocker | `canonical_object_hash_version` 入 Statement schema;ConflictProbe 只同版本直接比较,跨版本降级 review,canonicalize 升级走 maintenance rebuild | §3.2、§5.2、§16.1 |
| B3 | Blocker | 跨 tenant 派生例外规则:org/system 前驱可审计继承请求方 tenant;多 user tenant 需协议并进入 review;无协议拒绝 | §3.1、§5.1、§16.1 |
| M1 | Major | `derived_depth` 缓存派生链深度,保留 `derived_from` 为唯一前驱字段,Replay/ToM 用 O(1) 深度判断 | §3.2、§9.2、§10.2 |
| M2 | Major | BusEvent envelope 增 `primary_id`;一次性业务幂等键默认从 `primary_id + target_state/dimension_version/outbox_sequence` 推导 | §5.3、§5.4 |
| M3 | Major | testing helper production 防线:包级隔离 + `__starling_testing_only__ = True` + CI 静态扫描 + production preflight fail-closed | §4、§16.1 |
| M4 | Major | v16 内收口:补 hash 升级期双查/保守协议、SUPERSEDES 不累计 `derived_depth`、`idx_statement_id_tenant`、事件 aggregate 默认表、投影 consolidated/superseded 订阅与 testing 静态防线 | §3.2、§4、§4.1、§5.2、§5.3、§10.2、§16.1 |
| m1 | Minor | P0 temporal_anchor None、tenant/provenance 不可变、跨 tenant 派生、chunk 幂等等新不变量进入负面验收用例 | §5.1、§16.1 |
| m2 | Minor | 附录 D 补 v16 FAQ,解释 hash/version、derived_depth、testing helper、cross-tenant、group tenant 等实施规则 | 附录 D |

### 17.17 v16 → v17 修复记录(本版本)

> 本节基于 Section 1+2 plan-eng-review,v17 是 P0 编码起点。Section 1+2 14 项契约决定 + 3 项策略决定全部进文档,代码与文档同步。

**修复批次类型**:Section 1+2 plan-eng-review 14 项契约决定 + cross-model tension 3 项策略决定。

**Major 级修复**(影响 P0 编码契约边界):

| # | 决定 ID | 修复内容 | 影响章节 |
|---|---|---|---|
| 1 | A1 | VOLATILE↔REPLAYING_CONSOLIDATING 振荡上限 + TTL 兜底 | §3.4 / §10.2 |
| 2 | A2 | Commitment BROKEN 计数 + RENEGOTIATED 链长上限 | §12.2 |
| 3 | A3 | derived_from 防抖路径在 §5.4 显式列出 | §5.4 |
| 4 | A5 | REPLAYING_CONSOLIDATING fallback timeout(对称 RECONSOLIDATING) | §3.4 / §5.6 |
| 5 | A6 | T5/T8 并发互斥:decay 路由经事件总线,per-stmt 顺序串行 | §3.4 / §10.4 |
| 6 | A8 | severe path 双路径:ProfileCapability cross_partition_transaction + saga | §4 / §11.2 |
| 7 | A9 | Commitment 反向保护事件 active_holding/released | §3.4 / §5.3 / §12.2 |
| 8 | A4+A7 | 默认值冻结 + system.runaway 入 §5.3 主表 | §5.3 / §5.4 / §10 |
| 9 | Q1 | statement.references_existing 闭环 §11.1 触发路径 2 | §5.3 / §11.1 |
| 10 | Q2 | holder_perspective Extractor 填值规则 | §5.1 |
| 11 | Q3a | mild correction provenance 不变 + confidence_history | §3.10 / §11.2 |
| 12 | Q3b | canonicalize_object 对 StatementRef 补 modality + inner_object_hash | §5.2 |
| 13 | Q4 | 文档级修复:DRY/术语/阶段标注/3 张 ASCII 图 | §5.3 / §7.1 / §14.0 |
| 14 | P2 | canonical_object_hash 升级 maintenance 超期告警 | §3.2 / §5.2 |

**策略级修复**(cross-model tension 解决):

| T# | 决定 | 影响 |
|---|---|---|
| T1 | 保持六态机不变 | §3.4 状态机不动,outside voice 提出的"三态合并"建议未采纳。理由:v4→v16 修复链是信任信号,SQLite enum 多三个值的 schema 税可接受。 |
| T2 | P0 时间表调整 2 周 → 6 周 | §16.1 重写时间表,新增 M0.0,Lane 并行性显式化。理由:Section 1+2 新增 19 条用例使 M0.7 工作量翻倍,诚实估算优于乐观估算。 |
| T3 | v17 是 P0 编码起点 | 本版本即 v17。Section 1+2 14 项决定全部进文档,代码与文档同步。理由:14 项决定如不进文档只在对话记录中,人重启后丢失上下文。 |

**遗留待办(写入 TODOS.md)**:
- Retrieval P4 多源 fan-out latency budget(P4)
- mem0/Letta/Graphiti baseline benchmark(P0 后)
- embedding 策略落定(P1)
- FANToM/ToMBench eval suite 集成(P1+)
- failure mode 集(P0+):LLM 不可用 / SQLite WAL 锁 / outbox dispatcher hang / testing helper 渗透 / preflight 失败

**未变更项**(显式声明保留):
- Cognizer / Statement / Container / Engram 核心 schema 不变
- 11 类 modality 不变
- Statement 六态枚举不变(T1)
- 9 种 QueryIntent / 7 个 Mentalizing Primitives / 5 个 Substrate Profile 不变
- §0.1 / §0.2 收敛原则与简单性原则不变
- 7 大差异点叙事不变
- 9 个开源系统消化原则与外部映射不变(§16.2)
...
1. **LLM 抽取成本与延迟**:Statement 抽取 + holder 归因 + Conflict 检测是高频操作。缓解:7B 级专用抽取模型 + 高价值场景升级大模型;批量抽取合并;抽取缓存(同一 EngramStore 内容不重复抽取)。
...
**v14 收敛约束**:
- `mem0 Memory / Graphiti Episode / Letta Block / cognee DataPoint / MemOS MemCube / memU WorkflowStep / claude-mem Observation` 都不得直接成为 Starling 核心表;只能映射为 Engram、Statement、Container、PipelineRun 或 Adapter metadata。
...
| 模式分离(反相似偏移) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | 🟡 | ✅ |
| 模式补全(PPR 图游走) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ |
| EngramStore 证据保留/合规擦除 | ❌ | 🟡 | ❌ | ❌ | 🟡 | ❌ | ✅(原档) | ✅(原档) | 🟡 | ✅(retention mode) |
...
| 说错无法追溯 | 信任崩塌但找不到根因 | supersedes 链保留完整历史 + EngramStore 证据指针 |
...
| `Engram` | 原始证据记录,由 `Bus.append_evidence` 创建,Statement evidence 必须引用它 | §6.0 |
...
| `IngestPolicy` | EngramStore 写入入口对 source_kind 的存储策略,如 STORE/NO_STORE/STORE_METADATA_ONLY/REQUIRE_REVIEW | §5 |
...
| `SourceSegment` | Engram 内的片段级源映射,记录 offset、speaker、role、observed_at 和 hash | §3.8 |
| `SourceSpanRef` | Statement 到 Engram 片段/episode/segment 的证据锚,用于追溯抽取来源 | §3.2 |
| `SourceKind` | EngramStore 输入来源分类,用于阻止 system_internal/observer_agent/replay_output 自污染 | §5 |
...
| outbox at-least-once 投递,幂等性由谁保证? | Bus outbox 和 Subscriber inbox 都持久化 `idempotency_key`;P0 只冻结 `commitment.fire` 与 `statement.recalled` 两类用户可见副作用的业务幂等窗口,其他事件默认一次性业务键。 | §5.4 |
| 其他事件的一次性业务幂等键怎么取? | 默认取 `(event_type, event.primary_id, target_state/dimension_version/outbox_sequence)`,Subscriber 覆盖必须注册到 Bus 配置。 | §5.3、§5.4 |
...
| 多个 Cognizer 共享 Engram,单方 purge 会影响其他人吗? | 需按 retention_mode、access_policy 与 legal_hold 判断;共享记录不能简单物理删除,生产底座需 refcount/key shredding。 | §3.8、§6.0、§16.2 |
...
| metadata_only EngramStore 能作为事实证据吗? | 只能作为辅助来源;高影响 Statement 需要 byte-preserving 或其他可信 evidence 才能自动 APPROVED。 | §3.8、§6.0 |
...
| RetrievalReceipt 的 `NEEDS_RAW` 会直接打开 EngramStore 原文吗? | 不会;它只是声明摘要不足,真正打开 raw evidence 前还要通过 retention、visibility、tenant/holder scope 检查。 | §13.6.1 |
| 为什么历史导入不能用系统当前时间解析 "today/last week"? | 因为相对时间属于源材料观察时刻,必须绑定 `TemporalAnchor.anchor_time`;缺锚时只能低置信待审。 | §3.2、§5.1 |
| P0 不传 `temporal_anchor` 会被拒吗? | 不会;P0 可由 `SourceSpanRef.observed_at` 序列化推导,P2 开启多片段后才强制持久化。 | §3.2、§5.1 |
...
| 单 tenant 部署为什么也要写 `tenant_id`? | 因为查询模板、索引和 future migration 都依赖 tenant 谓词;单租户只把值固定为 `default`,不省略字段。 | §3.1、§3.2、§4 |
| group 的 tenant 能从成员推导吗? | 不能;group Cognizer 必须显式声明 tenant,P0 只支持单 tenant group,跨 tenant group 进入 review 或拒绝。 | §3.1 |
| 跨 tenant 派生一定拒绝吗? | 不是;org/system 前驱可审计继承请求方 tenant,多 user tenant 需要显式协议并进入 review,无协议才拒绝。 | §5.1 |
| `provenance` 写入后冻结,review 提升怎么记录? | review/confidence/state 变化只改对应字段和审计事件,不改 provenance;只有 fork/supersedes 新 Statement 才有新 provenance。 | §3.10 |
| 为什么有 `derived_depth` 但没有恢复 `derivation_chain` 字段? | `derived_depth` 只是 O(1) 深度缓存,前驱事实仍只由 `derived_from` 表达,避免第二套派生链事实源。 | §3.2、§10.2 |
| `canonical_object_hash` 算法升级后旧数据怎么办? | 新写入使用新 `canonical_object_hash_version`,旧行保持原版本;跨版本只降级 review,后台 maintenance 异步 rebuild。 | §3.2、§5.2 |
| Value 是 list/dict 时能 JSON dump 后 hash 吗? | 不能;P0/P1 拒收复合 Value,应拆成多条 Statement 或建 EntityRef。 | §3.2、§5.2 |
| testing helper 如何保证不进生产? | helper 必须在隔离命名空间并声明 `__starling_testing_only__ = True`;CI 静态扫描 production import graph,production preflight 再扫描已加载模块和注册路由。 | §4、§16.1 |
| `RetrievalScopePlan` 为什么要记录 skipped scope? | 因为 sufficiency 短路不是全量检索,Receipt 必须解释为什么没查 graph/vector/engram raw。 | §13.1、§13.6.1 |
