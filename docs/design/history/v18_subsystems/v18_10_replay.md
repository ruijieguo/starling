...
Replay Scheduler 负责将 VOLATILE Statement 巩固到 CONSOLIDATED，并对长期记忆执行衰减、抽象、规则归纳、技能锻造。触发模式分三种：Online（在线）、Idle（空闲）、Sleep（深度）。它只经事件总线 emit `statement.decay_candidate`，不直接修改 state，与 [Reconsolidation Engine](v18_11_reconsolidation.md) 严格分工。
...
| `reconcile` | 冲突 Statement 集 | 推入 [Reconsolidation Engine](v18_11_reconsolidation.md) | 输入：CONSOLIDATED → REPLAYING_RECONSOLIDATING | — |
...
所有 Replay 巩固输出的固定 provenance 标签。Bus 写入后 emit `statement.derived`（非 `statement.written`），Replay Scheduler 不订阅该事件。参见 [Statement Bus](v18_05_bus.md)。
...
- [Statement Bus](v18_05_bus.md) — 事件写入与 replay_derived 路由
- [Affect Buffer](v18_06_hippocampus.md) — VOLATILE TTL 兜底判断、Idle 优先级影响
- [Reconsolidation Engine](v18_11_reconsolidation.md) — reconcile 操作的接收方与冲突仲裁
