...
- [~] **Task 5.1(进行中):** vendored zvec → `thirdparty/zvec`(submodule recursive 拉取);`CMakeLists.txt` `add_subdirectory` zvec 完整构建 + 链接 `zvec` 主库 + core_knn_* 索引插件(force_load);macOS arm64 构建验证(NEON)。**关卡:编译通过才进 5.2。**
  - **2026-06-14 独立验证(`/tmp/zvec-build`,v0.5.0,浅 submodule 361M):** configure 在 macOS arm64 + AppleClang 21 + cmake 4.3.2 **成功**(42.5s,生成 Makefile);ARM64 检测 OK、protobuf 3.21.12 found、diskann/rabitq macOS 自动跳过、SSE42/uring/fallocate 等 Linux 特性 failed 但有 F_FULLFSYNC fallback(非致命)。**集成注意点:** ① cmake 4.3.2 需 `-DCMAKE_POLICY_VERSION_MINIMUM=3.5`(zvec 旧 submodule googletest-1.10.0 的 cmake_minimum_required<3.5);② **zvec 自带 googletest 1.10.0 与 starling FetchContent googletest 1.15.2 target 冲突**,集成需隔离(zvec `BUILD_TESTING=OFF` + 让 zvec 不引入自己的 gtest,或 target 命名隔离);③ zvec C++ target=`zvec`(主库)+ core_knn_*(force_load)。build 验证(arrow/rocksdb/protobuf 编译数十分钟)进行中。
