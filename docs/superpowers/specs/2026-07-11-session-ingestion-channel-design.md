...
**⚠️ v3 amendment(D3=A,2026-07-12 实现期定谳 — 取代本文所有「worker 不外套 _lock」表述):** codex 的「worker 外套锁零收益」建议基于「remember 的锁很短」的假设。实现 + opus 审揭穿:`remember` 开 `BEGIN IMMEDIATE` **横跨整段 ~44s extraction**,所以**不持锁 = 单写者违规**(worker 与 tick/HTTP 并发写同一连接 → BEGIN 套 BEGIN 抛错/丢写)。用户裁定 **D3=A:worker 用 `self.remember` 持 `_lock`(尊重单写者)+ 重限流(`_ingest_throttle_s`,块间大睡让 tick/HTTP 有窗口)**。信号改为 `ingest_remember_ms_total`(持锁跑 extraction 的累计墙钟,不是「锁等待」)——正是「extraction 出锁 / 方案2」被证明该做的证据(实测:一块抽取 54s 内并发 tick 等 37.8s)。下文 §④/§⑥/§⑦ 的「不外套锁 / 锁等待埋点 / 不锁死」按此更正。
