...
- 不改 `starling.Memory`（C2 harness 自建 pipeline，避免 P2.e churn）；无 C++ 改动；无 migration。
...
- **回归红线**：**无 C++ 改动**（纯 scripts + 语料 + docs；ctest 505 不动）；无 migration（最高 0021）；单一 `starling_tests`；不改 `starling.Memory`/既有 harness 逻辑（C3 只补语料）；pytest 增 C1/C2/C3 自测,全绿。
...
- **无 C++ / 无 migration / 不改 Memory**——纯 Python scripts + 语料 + 文档。
