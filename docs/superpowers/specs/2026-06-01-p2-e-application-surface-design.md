...
struct PersonaView {
    ...
    int version = 0;
    std::map<std::string, std::vector<std::string>> dimensions;  // dimension → items
};
...
class Memory:
    @classmethod
    def open(cls, db_path, *, agent="self", tenant_id="default", extractor=None) -> "Memory":
    ...
    def remember(self, text, *, holder=None, source_kind="user_input", now=None) -> "RememberResult":
        # holder 默认 = self agent。append_evidence(EngramInput) → Extractor.extract → Statement。
        # RememberResult = {engram_ref, statement_ids: list[str], outcome: str}。extractor 为 None → 抛清晰错误。
...
**extractor helper**（`starling` 顶层；门面只收已构造好的 extractor 对象）：
- `starling.make_openai_extractor(model=…, base_url=…)`：生产，key 走 env `OPENAI_API_KEY`（**绝不入参 / 不 log / 不绑 Python 形参**）。
- `starling.make_stub_extractor(rules)`：离线 / 测试，确定性把已知输入映射成 Statement（quickstart 用）。
...
```python
mem = Memory.open(db_path, agent="alice", extractor=make_stub_extractor(rules))
...
- `make_stub_extractor` 须真能离线把 quickstart 的输入抽成 Statement（确定性 fake，复用既有 FakeLLM/test extractor 模式）。
