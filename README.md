...
A web observability + interaction surface: a **FastAPI** engine-API fronted by a **SvelteKit + Tailwind** UI, organized as a brain-region map. Pages span **interaction** (chat-with-memory `converse`, raw `remember` / `recall`, working-set render, commitment reminders), **cognitive inspection** (statement explorer, cognizer social graph, commitment state machine, and **Lens** provenance forensics for a single memory), **dynamics** (replay with a per-batch **dream log**, **lifecycle** state distribution, **decay forecast**, conflict probe), **vitals** (read-only observability: pump lag, queue depth, recall-receipt attribution), and an **overview + eval-report** landing. Live updates stream over WebSocket; inspection panels read SQLite read-only, while commands — `converse`, `remember`, `tick`, plus interventions (`forget`, `review`, `replay_trigger`, `reconsolidate`) — run through the engine (with UI-configurable models).
...
**Configure models in the UI:** the dashboard starts with no model configured; inspection panels, `recall`, `tick`, and working-set work immediately (offline embedder), while `remember` (statement extraction) returns 409 until you set one. Open **Settings** (`/settings`) to manage a **named provider registry** (each entry: provider / model / base URL / API key) and bind each **role** — `extraction`, `embedding`, `chat`, `consolidation` — to a provider. `extraction` and `embedding` are wired today; `chat` drives `converse`. Changes are hot-swapped — no restart. Changing the embedding provider re-embeds existing memories (the vector dimension changes).

**Unified config — `~/.starling/starling.json` (0600):** one file holds everything — `db_path` (defaults to `~/.starling/dashboard.db`, auto-created), `agent`, `tenant`, `token` (auto-generated), `host`, `port`, `cors_origins`, the named-provider registry (`providers`), and the per-role bindings (`roles`), including each provider's API key. Legacy single-`{llm, embedder}` files are auto-migrated on load, so existing configs keep working with no manual edit. The file is gitignored and chmod-0600. Override its path with `STARLING_CONFIG` or `--config`; environment variables (`STARLING_DASH_*`, `OPENAI_*`) still override the file for CI / ephemeral use.
